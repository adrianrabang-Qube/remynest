...
export default function RemyTimeline({
  ...
  return (
    <section className="rounded-3xl border border-sand-deep/70 bg-white p-4 md:p-6 shadow-soft">
      ...
      <ol className="mt-5 max-md:mt-4 space-y-6 max-md:space-y-3 border-l border-sand-deep/70 pl-6">
        {groups.map((group) => (
            ...
            <ul className="mt-2 space-y-2 max-md:space-y-1">
              {group.events.map((event) => (
                <li key={event.id}>
                  <Link
                    href={event.href}
                    className="block rounded-2xl px-3 py-2 transition hover:bg-sand/40 max-md:py-1.5"
                  >

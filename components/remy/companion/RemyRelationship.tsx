  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Question Understanding — the deterministic retrieval-intent layer a FUTURE conversational
        // layer will use to turn a parsed question into structured intent over the real intelligence
        // stack (internal, never shown; NOT chat/GPT/LLM, takes no free-text). A memory in a strong
        // retrieval-intent focus feeds the significance engine.
        ...
        const questionStrengthByMemoryId = new Map<string, number>();
        for (const intent of questionUnderstanding.intents) {
          for (const id of intent.focus.memoryIds) {
            questionStrengthByMemoryId.set(
              id,
              Math.max(questionStrengthByMemoryId.get(id) ?? 0, intent.weight),
            );
          }
        }

        // Answer Planning — the deterministic execution plan a FUTURE conversational layer will run
        // after Question Understanding: ordered structured retrieval steps + the real source pool
        // (internal, never shown; NOT chat/GPT/LLM, produces no answers). A memory targeted by a strong
        // retrieval step feeds the significance engine.
        ...
        const answerPlanStrengthByMemoryId = new Map<string, number>();
        for (const step of answerPlan.steps) {
          for (const id of step.memoryIds) {
            answerPlanStrengthByMemoryId.set(
              id,
              Math.max(answerPlanStrengthByMemoryId.get(id) ?? 0, step.weight),
            );
          }
        }

        // Answer Assembly — the FINAL deterministic intelligence layer: assembles the factual answer
        // package (sections/chronology/evidence/coverage) a FUTURE conversational layer will verbalize
        // (internal, never shown; NOT chat/GPT/LLM, generates no answers). A memory backing a strong
        // assembled section feeds the significance engine.
        ...
        const answerAssemblyStrengthByMemoryId = new Map<string, number>();
        for (const section of answerAssembly.sections) {
          for (const id of section.memoryIds) {
            answerAssemblyStrengthByMemoryId.set(
              id,
              Math.max(answerAssemblyStrengthByMemoryId.get(id) ?? 0, section.weight),
            );
          }
        }
        ...
        const significant = rankSignificantMemories(datedMemories, {
          ...
          questionStrengthByMemoryId,
          answerPlanStrengthByMemoryId,
          answerAssemblyStrengthByMemoryId,
        });

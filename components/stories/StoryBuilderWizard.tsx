...
import { createStory } from "@/app/(app)/activities/stories/actions";
import MomentOrderList, {
  type MomentListItem,
} from "@/components/stories/MomentOrderList";

/**
 * Story Builder create wizard: SELECT 2–12 moments from the active workspace's
 * memories → ARRANGE (title + order) → save. Reuses the EXISTING paginated
 * /api/memories feed (server-scoped by the validated workspace cookie; signed
 * thumb variants; `data.data` response shape) — one tile per memory, photo
 * thumb when it has one, a calm serif text tile otherwise. No AI, no timers.
 */
...
export default function StoryBuilderWizard() {
  ...
  // ---- picker (same settle-always pattern as the puzzle wizard) ----
  ...
  const toggle = useCallback((id: string) => {
    ...
    setSelectedIds((prev) => {
      ...
      if (prev.length >= STORY_MAX_MOMENTS) return prev;
      return [...prev, id];
    });
  }, []);
  ...
  const orderedItems: MomentListItem[] = selectedIds
    ...
    .map((m) => ({
      ...
      title: (m as PickerMemory).title || (m as PickerMemory).excerpt || "Untitled moment",
  ...
  const onSave = useCallback(async () => {
    ...
    setError("");
    void haptic("medium");
    const result = await createStory({ title, memoryIds: selectedIds });
    if (result.ok && result.storyId) {
      void hapticSuccess();
      router.push(`/activities/stories/${result.storyId}`);
      return;
    }
    setSaving(false);
    setError(
      !result.ok && result.reason === "unavailable"
        ? "Stories aren't quite ready yet — please try again soon."
        : "We couldn't save your story. Please check the title and try again.",
    );
  }, [saving, title, selectedIds, router]);

  const canContinue = selectedIds.length >= STORY_MIN_MOMENTS;
  const canSave = canContinue && title.trim().length > 0 && !saving;
    ...
    <div className="mx-auto w-full max-w-2xl px-4 py-8">
      ...
      <header className="mt-4">
        <h1 className="font-serif text-2xl font-semibold text-charcoal md:text-3xl">
          {step === "select" ? "Choose your moments" : "Arrange your story"}
        </h1>
        <p className="mt-1 text-charcoal-soft">
          {step === "select"
            ? `Pick ${STORY_MIN_MOMENTS} to ${STORY_MAX_MOMENTS} memories — photos or words.`
            : "Give it a title and put the moments in the order that feels right."}
        </p>
      </header>
        ...
        <section className="mt-6">
          {loading ? (
            ...
            <div className="rounded-3xl border border-sand-deep/70 bg-white p-8 text-center shadow-soft">
              ...
              <p className="mt-1 text-sm text-charcoal-muted">
                Add a few memories first — then weave them into a story.
              </p>
            </div>
          ) : (
            <>
              ...
              >
                ...
                {canContinue
                  ...
                  : `Choose at least ${STORY_MIN_MOMENTS} moments`}
              </button>
            </>
          )}
        </section>
      ...
      {step === "arrange" && (
        <section className="mt-6 space-y-5">
          ...
          <MomentOrderList
            items={orderedItems}
            onMove={move}
            onRemove={selectedIds.length > STORY_MIN_MOMENTS ? remove : undefined}
          />

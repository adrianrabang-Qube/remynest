...
/**
 * The Nest menu — the bottom sheet Remy "presents" when the Nest opens. Pure presentation over
 * EXISTING routes (no new AI/conversation). Every href arrives pre-threaded with `?context=` from
 * the parent, so care-workspace routing is unchanged.
 *
 ...
 * corruption). So the overlay is `createPortal(document.body)`, mirroring the WorkspaceSelector
 * drawer and the prior RemyActionButton sheet. Focus-trap + Escape + focus-restore + scroll-lock
 * reuse the shared `useFocusTrap` hook; a desktop-breakpoint safety-close avoids a stranded
 * scroll-lock on resize.
 */
export default function NestMenu({
  ...
  return createPortal(
    ...
    >
      ...
      {/* Bottom sheet — Remy presenting the options */}
      <div
        ref={panelRef}
        tabIndex={-1}

...
Branch: main — **unpushed**, ahead of `origin/main` (`f53694b`); latest = the Journey Engine (on top of Memory-Graph + Memory-Understanding + Emotional-Intelligence + Living-Relationship + Companion-Intelligence + app-wide-companion + living-Nest + Nest-hub + doc-sync)
Commit: Journey Engine (on top of `d6cfb9c` Memory-Graph — see git log)
...
- ✔ **Pipeline wired in RemyRelationship**: snapshot → **memory-understanding → memory-graph → journey** → story → favourite → anniversary → significance → emotional → personality → relationship → priority → one `<Remy>` renderer. Snapshot enriched (real `attachments`/`ai_importance`/`memory_person_links`/historical flag).

...
Branch: main — **unpushed**, ahead of `origin/main` (`f53694b`); latest = the Memory Understanding Engine (on top of Emotional-Intelligence + Living-Relationship + Companion-Intelligence + app-wide-companion + living-Nest + Nest-hub + doc-sync)
Commit: Memory Understanding Engine (on top of `cc768a9` Emotional-Intelligence — see git log)
...
- ✔ **Pipeline wired in RemyRelationship**: snapshot → **memory-understanding** → story → favourite → anniversary → significance → emotional → personality → relationship → priority → one `<Remy>` renderer. Snapshot enriched (real `attachments`/`ai_importance`/`memory_person_links`/historical flag).

...
Branch: main — **unpushed**, ahead of `origin/main` (`f53694b`); latest = the Emotional Intelligence Engine (on top of Living-Relationship + Companion-Intelligence + app-wide-companion + living-Nest + Nest-hub + doc-sync)
Commit: Emotional Intelligence Engine (on top of `ccfb907` Living-Relationship — see git log)
...
- ✔ **Pipeline wired in RemyRelationship**: snapshot → story → favourite → anniversary → **significance → emotional → personality** → relationship → priority → one `<Remy>` renderer. Snapshot enriched (real `attachments`/`ai_importance`/`memory_person_links`/historical flag).
...
- **Decision:** Emotional Intelligence = three PURE engines (`significance-engine` ranks by SIGNIFICANCE not recency, `emotional-engine` → `EmotionalProfile`, `personality-engine` → behavioural traits). The 0–100 scores are INTERNAL — **Remy never exposes raw scores, only behaviours/observations/traits**. Pipeline order is FIXED: snapshot → story → favourite → anniversary → significance → emotional → personality → relationship → priority → one renderer. **Status:** LOCKED. **Do NOT** render a raw score, rank memories by recency/creation-order, put clock/DB in the engines, or fabricate significance from absent data.

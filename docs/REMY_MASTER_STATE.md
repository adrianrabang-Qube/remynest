...
Branch: main — **unpushed**, ahead of `origin/main` (`f53694b`); latest = the Memory Graph Engine (on top of Memory-Understanding + Emotional-Intelligence + Living-Relationship + Companion-Intelligence + app-wide-companion + living-Nest + Nest-hub + doc-sync)
Commit: Memory Graph Engine (on top of `63e944e` Memory-Understanding — see git log)
...
- ✔ **Pipeline wired in RemyRelationship**: snapshot → **memory-understanding → memory-graph** → story → favourite → anniversary → significance → emotional → personality → relationship → priority → one `<Remy>` renderer. Snapshot enriched (real `attachments`/`ai_importance`/`memory_person_links`/historical flag).

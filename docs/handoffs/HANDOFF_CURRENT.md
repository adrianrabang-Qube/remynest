...
- **Unified Remy Bird branding — Option-1 identity-only (2026-06-23, NOT pushed).** SUPERSEDES the two-tier sage-product/purple-companion brand. ONE brand = the purple/gold **Remy Bird** (mascot = companion = product mark). **Identity layer** (purple `#8A6BD0`→`#5B3E8E` / gold `#C9A86A` / navy `#2A2350`) on logo/icon/favicon/OG/splash/auth; **app chrome (sage/sand/charcoal) UNCHANGED — no app recolor** (Option 2 deferred). **Created/replaced:** all 7 `public/brand/logo-*.svg` masters (geometric bird-in-nest-rings + heart; `logo-mark-dark.svg` = the raster master); `app/icon.svg` (bold favicon bird); `app/apple-icon.tsx` + `app/opengraph-image.tsx` (rewritten → bird on purple, OG tagline "A safe place for your memories"); `components/brand/RemyBirdLogo.tsx` (shared lockup) added to **login + signup** (which also got `bg-black`→`bg-sage` buttons); `public/brand/splash.svg` (splash master); `lib/brand/tokens.ts` (new `IDENTITY` tier, doctrine comment flipped, `BRAND`/`REMY` kept); `scripts/generate-brand-assets.mjs` (composites onto **purple** now). Updated `docs/brand/{brand-guidelines,asset-production}.md` + the **CLAUDE.md brand block** (authoritative). **FROZEN sprite** `public/remy/remy-blueprint.png` untouched. Lint **0 new** (67=baseline), build **✓**. **⚠️ Operator/designer follow-ups (cannot do here — no `sharp`/native/designer):** run `scripts/generate-brand-assets.mjs` → PWA/store/Play/maskable/favicon PNGs; wire native iOS/Android icons + splash (`@capacitor/assets` + `cap sync`); **replace `public/logo.png`** (raster wordmark, still sage — referenced by `lib/seo.ts` `SITE_OG_IMAGE` + `app/page.tsx` JSON-LD, so the *static* OG still shows the old logo until regenerated); retire the unreferenced sage `public/brand/remy-*.svg` state SVGs; polished designer bird illustration may later supersede the geometric SVGs.

...
## Active branch
`main` (production; auto-deploys). **Native iOS Local Notifications for reminders
landed on `main`** (this commit) — web code auto-deploys (no-op in browsers); iOS
activation needs the operator's `pod install` + native build + device test (see
`docs/features/local-notifications.md`). `cognition-v2` was the throwaway prototype
branch (stale UI, 180 commits behind) and is **not** to be merged.
`feat/capacitor-mobile` holds earlier mobile work (pushed, unmerged).

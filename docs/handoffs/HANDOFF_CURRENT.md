...
- **Master synchronization system established (workflow, 2026-07-09).** Created **`docs/REMY_MASTER_STATE.md`** — the single authoritative source of truth for project state (launch readiness ~90%, overall ~70%; current milestone = App Store Submission Readiness; VERIFIED-COMPLETE list, DO-NOT-REBUILD list, LOCKED architecture decisions, launch blockers split ENG/PRODUCT/LEGAL/OPERATOR, known open items, post-launch roadmap, next task). Populated from the 2026-07-09 10-agent source-verified sync audit. **New standing rule:** read `REMY_MASTER_STATE.md` FIRST every session and update it (+ this HANDOFF, + CLAUDE.md for architecture changes) after every completed task; **source code wins** over any doc. CLAUDE.md session-start protocol updated (step 0). **Single highest-priority engineering task before launch = UGC report/block + EULA abuse clause (Apple 1.2).** Key sync corrections captured: the AI stack (Ask Remy / semantic search / embeddings / summaries / tagging) is SHIPPED & live on OpenAI (only companion animation/voice is deferred); access_level enforcement, downgrade reconciliation, caregiver revoke, subscription→storage-tier, thumbnails/pagination, and Project Polaris (all 8 passes) are DONE; the center nav is the behaviour-driven Nest (RemyActionButton retired). Doc-only change (no code); iOS `project.pbxproj` build-bump left UNSTAGED.

- **The Nest — behaviour-driven correction (product, 2026-07-09, NOT pushed).** CORRECTED the 2026-07-08 Nest hub, which still behaved like a prettier FAB (idle→tap→**menuOpen**→close). It is now a **behaviour-driven** interaction living in the ONE Remy platform: **the interaction is the feature; the menu is a CONSEQUENCE of Remy greeting.** **New platform layer** (`lib/remy/core/`, exported via `@/lib/remy`): `behavior.ts` = `RemyBehavior` vocabulary (resting/sleeping/idle/waking/peeking/emerging/greeting/listening/thinking/searching/recording/celebrating/reminder/memoryFound/processing/success/returningHome) + `BEHAVIOR_LOOK` mapping each behaviour to an **EXISTING** expression/emotion/animation-cue (no new art) with a `presentsActions` flag; `nest.ts` = the Nest **choreography** (`NEST_WAKE_SEQUENCE` waking→peeking→emerging→greeting[sticky], `NEST_RETURN_SEQUENCE` returningHome→resting) **+ Nest EVOLUTION** (`NestStage` small→growing→blooming→family→legendary + pure `resolveNestStage`). **This is a NEW platform layer above expression/emotion — the requested extension, NOT a second brain/renderer/provider/registry/policy/event-bus** (existing emotion pipeline + floating presence byte-untouched). **Nav surface = thin PLAYER** (`components/navigation/nest/`): `use-nest-interaction.ts` rewritten to schedule the platform choreography (reduced-motion-safe; leak-proof `onBehaviorChange` seam), `Nest.tsx` rewritten (persistent stage-aware nest, behaviour-driven Remy via the single `<Remy state>` renderer, `float` while resting, `data-nest-stage` accent), `NestMenu.tsx` unchanged (portaled sheet shown WHILE greeting). **Deleted** the parallel `nest-state-machine.ts` (menuOpen states) + `nest-animations.ts`. Default = Remy **asleep in the nest** (`resting`→`sleeping`); tap **wakes** → peeks → climbs out → greets → presents 5 EXISTING routes (`/remy`, `/memories/new`, `/reminders`, `/search`, `/insights`, all `withContext()`-threaded) → choose → returns home → rests. Overlay stays `createPortal(document.body)`. **Future-ready** (voice/AI/celebrations/Golden Feather/seasonal/evolution/accessories/emotion/Watch/Widgets/CarPlay) by TRIGGERING a reserved behaviour + emitting an event — reserved behaviours defined+mapped but unreachable from the tap flow; **no deferred AI/voice/Rive/Lottie built**. **Presentation + routing ONLY** — billing/auth/caregiver+access_level/reconcile/reminders(scheduling/native/OneSignal)/memory/search/workspace/active-profile all byte-unchanged. Validation: tsc clean · lint 0 errors (2 pre-existing) · build ✓ · independent adversarial review CLEAN (all 15 verdicts YES — one-of-each preserved, emotion pipeline untouched, behaviour-not-menu-state, existing-vocab-only, portal + a11y + reduced-motion intact, no regressions). Docs updated same commit (CLAUDE.md authoritative note rewritten + this bullet). iOS `project.pbxproj` build-bump left UNSTAGED. **Flagged:** 'Remy Design Bible' not in repo (external); Remy PNGs opaque → white `variant='nest'` pedestal is the nest vessel (dedicated per-stage art = future registry-only drop).

- **The Nest — bottom-nav center interaction hub (product, 2026-07-08, NOT pushed).** Replaced the Polaris-Pass-1 static Remy action-sheet with **'The Nest'**: Remy rests in the nest (idle breathing + float + soft gold glow) and, on tap, **wakes → peeks → pops out → presents a menu → settles back** (calm timed sequence, reduced-motion-safe). Menu routes to **5 EXISTING surfaces** via `withContext()` — Ask Remy `/remy`, Add a memory `/memories/new` (`MOBILE_NEW_ACTION.href`), Add a reminder `/reminders`, **Search `/search`** (new to the hub), **Insights `/insights`** (new to the hub; both also kept in the 'More' drawer). **New `components/navigation/nest/`:** `nest-state-machine.ts` (pure interaction FSM — `NestPhase` idle/peek/popout/menuOpen/returnHome + reserved listening/thinking/processing/celebrate; `NEST_VISUALS` = the single phase→existing-`RemyExpression` map; pure `nestTransition`), `nest-animations.ts` (timing), `nest.module.css` (glow/wake keyframes, reduced-motion guards), `use-nest-interaction.ts` (React binding: timed advances, reduced-motion skip, leak-proof `onPhaseChange` seam — NOT wired to the platform), `NestMenu.tsx` (portaled bottom sheet), `Nest.tsx` (trigger + idle pedestal). `MobileBottomNav.tsx` now renders `<Nest>` (5 hrefs); `RemyActionButton.tsx` **deleted**. **Platform one-of-each preserved** — Remy drawn ONLY via the single `<Remy state>` renderer (no hardcoded `<img>`), overlay `createPortal(document.body)` (backdrop-blur invariant), NO second renderer/provider/registry/public-API/event-bus/brain/policy; the FSM is UI-interaction state, not a second emotion brain. **Presentation + routing ONLY** — billing/auth/caregiver+access_level/downgrade-reconcile/workspace/active-profile/reminders (scheduling/recurrence/timezone/delivery/OneSignal/NativeReminderSync/form-reset)/memory-engine/search all byte-unchanged. **No deferred content built** — no Rive/Lottie/voice/conversation/AI; reserved FSM states defined+mapped but unreachable from the tap flow. **Flagged constraints:** the referenced 'Remy Design Bible' is NOT in the repo (external; visuals scoped to existing approved assets + CSS); Remy PNGs have opaque backgrounds so the clean white `variant='nest'` pedestal is the nest vessel (true nest-compositing + `nestOpen/nestClosed` art await transparent assets = a future registry-only drop, no code change). Validation: tsc clean · lint 0 errors (2 pre-existing) · build ✓ · independent adversarial review CLEAN (all 16 verdicts YES — every DO-NOT-CHANGE system unchanged, portal + a11y + reduced-motion intact, no regressions). Docs updated same commit (CLAUDE.md authoritative Nest note + this bullet). iOS `project.pbxproj` build-bump left UNSTAGED.

- **Caregiver `access_level` enforcement — final V1 caregiver access-control gap CLOSED (product, 2026-07-08, NOT pushed).** An accepted caregiver's WRITE permission is now determined ONLY by `access_level` (`read`|`full`|`admin`; invite default `full`) — previously every accepted caregiver had full write. Model: **Owner → full write always; accepted caregiver → write ONLY per `access_level`; read-only caregivers keep enter+read.** **Single source of truth** in `lib/profile-ownership.ts` (no duplicated authz): private `resolveProfileRole(userId, profileId)` = the ONE query path (owner `created_by_account_id`, else ACCEPTED `profile_relationships` row + `access_level`, else none); `accessLevelCanWrite(level)` = the ONE permission rule — **only `read` restricts** (`level !== 'read'`), so `full`/`admin`/null/legacy stay write-capable (no-regression). `userCanAccessProfile` (access/enter/read — external behavior **byte-identical**: owner OR accepted → true) and new `userCanWriteProfile` (owner OR caregiver-with-write-level) both derive from `resolveProfileRole`. **Enforcement: 5 WRITE sites swapped `userCanAccessProfile` → `userCanWriteProfile`** (authorization call only) — `createReminder` + reminder toggle/complete + reminder delete (`app/(app)/reminders/page.tsx`), `POST /api/create-reminder`, `POST /api/memories/create`. **READ/enter paths UNCHANGED** (`getActiveContext`, `reminders/[id]` view, `getAccessibleProfiles` keep `userCanAccessProfile`) so read-only caregivers still enter+view. Memory edit/delete (`PUT`/`DELETE /api/memories/[id]`) is `user_id`-scoped (owner-of-content) — not access-level-gated, unchanged. Invite/list/revoke stay `userOwnsProfile` owner-only. **The FROZEN reminder scheduling/delivery/native-sync/OneSignal/recurrence/form-reset logic is byte-unchanged** — authorization-only swap (operator-approved under the freeze's authz-hardening carve-out). `userId` session-derived everywhere; `access_level` read server-side (service-role), never client input (no IDOR/escalation). Validation: tsc clean · lint 0 errors (2 pre-existing) · build ✓ · independent adversarial review CLEAN (all 12 verdicts YES: owner preserved · read-only blocked at all 5 writes · full/admin/null still write · read-only keeps enter+read · reminder behavior byte-preserved · no IDOR · single source). **Caregiver V1 access-control is now COMPLETE** (invite → accept/decline → manual revoke → downgrade auto-reconcile → access_level enforcement). **Remaining caregiver roadmap:** none for V1 access control; future = finer-grained per-surface permissions + UI reflection (hide write controls for read-only caregivers; server enforcement is already authoritative). iOS `project.pbxproj` build-bump left UNSTAGED.

- **Subscription downgrade entitlement reconciliation — V1 launch gap closed (product, 2026-07-08, NOT pushed).** Implemented the audit-identified reconciliation so a downgrade that removes the caregiver-collaboration entitlement automatically withdraws access (complements the manual owner-only revoke). **`lib/billing/reconcile-entitlements.ts`** (new): `reconcileEntitlementsForUser(userId, plan)` — service-role, **structured result, never throws, idempotent**. When the NEW plan lacks caregiver collaboration (`getUsageLimits(plan).caregiverCollaborationEnabled` — single source of truth `BILLING_PLANS[plan].caregiverCollaboration`; only FAMILY/ENTERPRISE grant it), it bulk-DELETEs accepted, non-owner `profile_relationships` rows on the profiles the user OWNS (`created_by_account_id`), preserving the owner's own row. **Non-destructive** (only access grants — never a care profile/memory/reminder), **no escalation** (only deletes access; never grants/upgrades/writes plan). **Webhook** (`app/api/stripe/webhook/route.ts`, additive only): reconciliation runs AFTER the existing profile write in `customer.subscription.deleted` (→FREE) and `customer.subscription.updated` (`!isActive?"FREE":derivedPlan??null` — null=unknown price=skip); a reconciliation error sets the existing `writeFailed` flag → the webhook's existing 500 retry (idempotent). `userId` comes from the webhook's own write result (`data.id`), never client input. **All other premium capabilities** (semantic search, storage quota, care-profile creation limit, voice) are read-time-derived from the persisted plan (`resolveSubscription`/`getUsageLimits`) → auto-reconciled by the plan write; existing over-limit care profiles are intentionally NOT deleted (creation-time enforcement only — deletion = destructive data loss, not a repo rule). **Regression-safe:** the webhook's existing writes / `writeFailed` semantics / 200-500 responses / checkout / portal / payment-failed grace are byte-unchanged; caregiver-revoke / reminders / memory / Polaris / Remy button / auth all untouched. Validation: lint 0 errors (2 pre-existing) · tsc clean · build ✓ · independent adversarial review (no escalation / no data loss / additive-only verified). **Remaining caregiver roadmap:** `access_level` enforcement (stored/shown, not enforced). iOS `project.pbxproj` build-bump left UNSTAGED.

- **Caregiver revoke — owner-only access removal (V1 launch gap closed) (product, 2026-07-08, NOT pushed).** Implemented the audit-identified missing revoke flow (an owner could invite/accept caregivers but never REMOVE access). **Server actions** (`app/(app)/dashboard/actions.ts`): `revokeCaregiver({memoryProfileId, caregiverAccountId})` + `listProfileCaregivers(memoryProfileId)` — **owner-only** (`userOwnsProfile`, session-derived user id, never a client id), **structured results (never throw)**, service-role (`supabaseAdmin`) reads/writes **scoped to the owned profile**. Revoke DELETEs the accepted `profile_relationships` row (scoped `memory_profile_id + caregiver_account_id`, `.neq('relationship_type','owner')`) → access withdrawn immediately (getAccessibleProfiles/userCanAccessProfile require an accepted row; getActiveContext re-validates the cookie on read → a revoked caregiver falls back to My Nest next request). Guards: **self-revoke blocked** (`caregiverId === user.id`), **owner row never deleted**, **non-owner blocked before any write**; **NOT entitlement-gated** (revocation works post-downgrade — the owner's manual remedy for the "downgrade doesn't revoke" gap). **UI:** new `components/CaregiverManager.tsx` (lists name/email/access-level + Remove w/ inline confirm + loading/success/error, Polaris design, renders nothing for non-owners) wired into the `WorkspaceSelector` "Manage care profiles" panel; telemetry `caregiver_revoked`. **Regression-safe:** invite/accept/decline/createProfile/workspace-switching/memories/reminders/billing/auth all byte-unchanged (additive only). Validation: tsc clean · lint 0 errors (2 pre-existing) · build ✓ · independent adversarial review (owner-only + no authorization bypass verified). **Remaining caregiver roadmap:** Stripe downgrade auto-revoke (manual revoke now exists); `access_level` enforcement (stored/shown, not enforced). iOS `project.pbxproj` build-bump left UNSTAGED.

- **Project Polaris — Pass 8: Reminders — FINAL PASS (product, 2026-07-08, NOT pushed).** Presentation-only refinement of the **CRITICAL FROZEN** reminder surface (NO reminder logic/creation/editing/deletion/completion/delivery/OneSignal/NativeReminderSync/AppDelegate/timezone/cron/background-task/API/Supabase-mutation/auth/routing/**form-reset** change; adversarially verified — every flow UNCHANGED). The reminder system stays SECURITY/PRODUCTION FROZEN; only visual presentation changed. The surface was already brand-aligned → a light refinement (**4 files**). **Black boxes (untouched):** `NativeReminderSync`, `NativeReminderBeacon`, `ReminderDateTimeField` (pure timezone logic — only its parent-passed className got `text-base`), `lib/native-reminders.ts`, `lib/reminders/*`, `app/api/reminders/*`. **`app/(app)/reminders/page.tsx`** (return only — the `createReminder`/`toggleReminderComplete`/`deleteReminder` server actions, the **form `key` reset**, the `NativeReminderSync`/`NativeReminderBeacon` mounts, and all input `name`s **byte-identical**): `max-w-3xl` column, **serif h1** (`text-4xl`→`text-2xl md:text-3xl`), summary focus ring + reduced-motion chevron, **`text-base` inputs** (title/date/frequency — no iOS zoom), submit focus ring. **`ReminderCenter.tsx`** (frozen `useMemo` grouping/lifecycle/priority, `useState(showCompleted)`, toggle/delete forms + hidden `id`/`completed` inputs): the one raw hex `text-[#9c7e3f]` → `text-gold-ink`, hero `rounded-3xl` + **serif "Next up" title**, card + button reduced-motion + additive sage/rose focus rings (`rose`/`gold` remain the approved urgency/destructive palette). **`reminders/[id]/page.tsx`**: restyled the existing debug view (container + serif h1 + brand `<pre>`; no new nav; auth/fetch/`notFound` frozen). New **`app/(app)/reminders/loading.tsx`** brand skeleton (reduced-motion-safe). Validation: brand sweep CLEAN · tsc/lint/build green · adversarial review (all 16 flows UNCHANGED). **PROJECT POLARIS COMPLETE — the app-wide presentation redesign (Passes 1–8) is finished.** Authoritative note in `CLAUDE.md`. iOS `project.pbxproj` build-bump left UNSTAGED.

- **Project Polaris — Pass 7: Settings (product, 2026-07-08, NOT pushed).** Presentation-only redesign of the Settings surface (NO profile-update/GDPR-export/account-delete/reauth/auth/routing/state change; adversarially verified — SETTINGS LOGIC UNCHANGED). Scope-locked to **7 files**; **`StorageUsageCard`, `ProfileHeader`, `ProfileSection` kept as black boxes** (shared / purchase-adjacent). **`app/(app)/settings/page.tsx`**: fixed a **nested `<main>`** (→`<div>`; the `(app)` layout already provides `<main>`), added a semantic `<header>` + **serif h1** + subtitle, storage wrapper `rounded-3xl`; `resolveAccountIdentity`/`redirect`/composition unchanged. **`AccountInformationSection`** (frozen `handleSave`/`PATCH /api/profile`/state): `neutral`→`charcoal`, `rounded-2xl` cards, **`text-base` inputs + sage focus rings**, `bg-black` save → sage (≥44px), `green/red` status → `sage-deep`/`rose`. **`ExportDataSection`** (frozen `handleExport`/`GET /api/gdpr/export`/blob): `bg-black` → sage button, rose error. **`PrivacyLinksSection`**: brand cards + focus rings (hrefs frozen). **`DeleteAccountSection`**: red → brand **rose** danger palette (approved), ≥44px + focus ring (modal trigger frozen). **`DeleteAccountModal`** (CRITICAL — presentation-only): `bg-black/50` scrim → `bg-charcoal/40`, red → **rose** destructive, neutral/gray → charcoal, `rounded-3xl` surfaces, **`text-base` password/typed inputs + sage focus rings**, ≥44px Cancel/Delete, and **additive `role="dialog"`/`aria-modal`/`aria-label`** — **every** `useState`/`useEffect`/`canSubmit`/`reauthOAuth`/`handleDelete`/`GET`+`DELETE /api/gdpr/delete-account`/`signOut`/`window.location`/typed-confirmation/`password`/`deleteContributed` binding **byte-identical** (verified: logic diff empty; NO focus-trap/Escape added — keyboard behaviour unchanged). New **`app/(app)/settings/loading.tsx`** brand skeleton (reduced-motion-safe). Validation: brand sweep CLEAN (rose = approved destructive) · tsc clean · lint 0 errors (2 pre-existing) · build ✓ · independent adversarial review (SETTINGS/PROFILE/EXPORT/DELETE/REAUTH/AUTH/ROUTING all UNCHANGED). **Polaris roadmap remaining:** Reminders (presentation only). Authoritative note in `CLAUDE.md`. iOS `project.pbxproj` build-bump left UNSTAGED.

- **Project Polaris — Pass 6: Library (product, 2026-07-08, NOT pushed).** Presentation-only redesign of the Library discovery surface (NO queries/filtering/routing/hrefs/data-loading/auth/state change; adversarially verified — LIBRARY LOGIC UNCHANGED). The Library was already ~90% brand-aligned, so this was a light refinement (**6 files**, scope-locked; the shared `Remy*` renderers treated as black boxes and left untouched). **`components/library/LibraryView.tsx`** (a pure client-side filter over a static 6-destination `SECTIONS` list — no data logic): the one off-brand `bg-stone-50` → `bg-sand`; the search input `text-sm`→**`text-base`** (no iOS zoom) as a **rounded-full pill** with a sage `focus-within` ring; filter chips got sage focus rings + `px-0.5 py-1` ring-clearance; the destination list → `rounded-3xl` + `shadow-soft` with a sage focus ring on each row `<Link>`; the bare empty-state `<p>` → a calm centered card. `SECTIONS`/`CHIPS`/hrefs/`useState`/filter predicate **byte-identical**. **`app/(app)/library/page.tsx`**: `<div>`→semantic `<header>`, **serif h1**, `mx-auto max-w-2xl` reading column, warmer "personal archive" subtitle. **`story`/`biography`/`memory-book` `page.tsx`** (server; auth + `Promise.all`(getRemy…) + `deriveLifeJourney/StorySignals` + the `Remy*` renderer calls **byte-unchanged**): back-link got a sage focus ring, an additive **`sr-only` `<h1>`** gives each page a proper document heading without duplicating the renderer's own `<h2>`, and the empty-state `<p>` → a calm card. **New `app/(app)/library/loading.tsx`** — brand skeleton mirroring the hub, reduced-motion-safe. Validation: brand sweep CLEAN · tsc clean · lint 0 errors (2 pre-existing) · build ✓ · independent adversarial review. **Polaris roadmap remaining:** Settings, reminders-presentation. Authoritative note in `CLAUDE.md`. iOS `project.pbxproj` build-bump left UNSTAGED.

- **Project Polaris — Pass 5: Timeline (product, 2026-07-08, NOT pushed).** Presentation-only redesign of the Timeline into a calm life-story surface (NO timeline logic/query/sorting/date-grouping/chronology/pagination/`signMemories`/AI-enrichment/navigation/routing change; adversarially verified — TIMELINE LOGIC UNCHANGED). Timeline was built pre-brand-system, so this was the largest rebrand yet — **13 files** taken from raw `gray/black/yellow/#f5f1e8` + oversized `text-4xl/5xl` titles → the Polaris system. **`app/(app)/timeline/page.tsx`** (server, presentation region only — auth, `getActiveContext`, pagination [PAGE_SIZE/SAFETY_CAP/isNarrowed/shown/fetchLimit], the memories query, filtering, `signMemories`, `groupMemoriesByDate`, categories query all **byte-unchanged**): `max-w-3xl` reading column; the `bg-yellow-50` no-profile notice, `bg-[#f5f1e8]` sticky bar, `border-gray-100`/`shadow-sm` empty state, and `text-gray-800` auth fallback rebranded; the empty state is now a calm serif **"Your story starts here"** + CTA; Show-more got a focus ring + ≥44px. **`TimelineHeader`** → serif title + warmer copy ("AI Timeline" → "Timeline · The story of your memories — newest first"). **`TimelineDayGroup`** sticky date headers → `bg-sand` + `charcoal-muted` uppercase labels (sticky offsets unchanged). **`TimelineCard`** (desktop `<details>`): brand rebrand, serif title (`text-4xl`→`text-2xl md:text-3xl`), calmer preview scale, `🕰 Memory Date:` → `CalendarClock` icon + `sr-only`, focus-ring on the summary, reduced-motion; native expand/collapse untouched. **`TimelineRow`** + **`TimelineAttachmentImage`** → focus ring / brand border. **Control bar** (`TimelineViewToggle` / `TimelineCategories` / `TimelineSearch`): `bg-black` active → `bg-sage`, gray → brand, `text-base` search input (no iOS zoom), sage focus rings, `aria-current`; every href/URL-builder + the `GET`-form search preserved. **Chapters view** (`ChaptersView` / `LifeChapterCard` / `RelatedMemories`): brand rebrand + serif + focus rings; hrefs/dedup/logic preserved. **`loading.tsx`** rewritten to mirror the new layout, brand-only, reduced-motion-safe. (`IntelligenceStrip` + `CompactChapterRow` were already brand-clean.) Validation: brand sweep CLEAN · tsc clean · lint 0 errors (2 pre-existing) · build ✓ · independent adversarial review. **Polaris roadmap remaining:** Library, Settings, reminders-presentation. Authoritative note in `CLAUDE.md`. iOS `project.pbxproj` build-bump left UNSTAGED.

- **Project Polaris — Pass 4: Search (product, 2026-07-08, NOT pushed).** Presentation-only redesign of the global search surface (no search logic/query/embeddings/ranking/filter/pagination/debounce/keyboard change; adversarially verified — SEARCH LOGIC UNCHANGED). **Search page** (`app/(app)/search/page.tsx`): replaced the `sr-only` h1 with a **visible serif title + inviting subtitle** ("Find a memory, a person, or a moment — just start typing"); `RemySearchInsights` + `SearchView` unchanged. **`SearchView`** (`components/search/SearchView.tsx`, presentation only): the field became an **inviting rounded-full pill** with a leading icon + sage `focus-within` ring (already `text-base`/no-iOS-zoom, clear button, `Remy.emit`, 250ms debounce + `AbortController` all untouched); filter/recent/suggestion chips got **sage focus rings + roomier `px-4 py-2`**; a **calm discovery hero** (`RemyStage context="welcome"` + "What are you looking for?" + a plain-language invitation) now leads the empty state above recent+suggestions; group-collapse buttons got focus rings + a reduced-motion chevron. **`SearchResultRow`**: the off-palette amber/sky/violet/rose type tints were neutralized to a single calm **brand sage** (type still conveyed by icon + badge text — "hierarchy over colour"), plus a sage focus ring. **`search/loading.tsx`** rewritten to mirror the new layout (title + insights + pill field + hero), brand tokens, reduced-motion-safe. **The debounced live search, `/api/search/global` fetch, abort-on-new-query, semantic/ranking/filtering/pagination, Enter/onSubmit→`commitRecent`, and localStorage recent-searches are byte-unchanged** (verified: search-logic diff empty). Validation: tsc clean · lint 0 errors (2 pre-existing) · build ✓ · independent adversarial review. **Polaris roadmap remaining:** Timeline, Library, Settings, reminders-presentation. Authoritative note in `CLAUDE.md`. iOS `project.pbxproj` build-bump left UNSTAGED.

- **Project Polaris — Pass 3: People + Insights (product, 2026-07-08, NOT pushed).** Presentation-only redesign (no data/query/logic change; adversarially verified — BEHAVIOR PRESERVED). **People** (`app/(app)/profiles/page.tsx`, return JSX only): serif header, calm `max-w-2xl` column, `rounded-3xl` list with subtle `divide-y` separators, polished empty state; `components/profile/people/PersonRow.tsx` gained a sage focus ring + roomier `px-3 py-3` (still ≥44px); **`AddPersonButton`** got a bigger touch target + focus ring and its modal now traps focus + Escape-closes + restores focus + locks scroll (shared `useFocusTrap`, `bg-charcoal/40` scrim) — the `CreateProfileForm` flow is unchanged; new `app/(app)/profiles/loading.tsx` skeleton. **Insights** (`components/insights/InsightsClient.tsx` + `app/(app)/insights/page.tsx` + `loading.tsx`): reframed from analytics-dashboard → **calm companion view** — the narrative layer stays VISIBLE (serif header [was `text-5xl`/`#243428`/gray], `AIDisclaimer`, `RemyInsightsCenter`, `AIInsightSummary`) and the **11 detailed charts collapse into a native-`<details>` "Detailed analytics"** progressive disclosure. **Every chart preserved + all `useMemo`/cognition-engine/analytics computation byte-unchanged** (verified: compute region 0 diff hunks); charts mount + render on expand (recharts re-measures). Off-brand `ChartSkeleton` / route skeleton / `bg-[#f5f1e8]` rebranded to tokens; width `max-w-7xl`→`max-w-4xl`. Validation: tsc clean · lint 0 errors (2 pre-existing) · build ✓ · independent adversarial review. **Polaris roadmap remaining:** Search, Timeline, Library, Settings, reminders-presentation. Authoritative note in `CLAUDE.md`. iOS `project.pbxproj` build-bump left UNSTAGED.

- **Project Polaris — Pass 2: Home + Memories (product, 2026-07-08, NOT pushed).** Presentation-only redesign of the two highest-traffic screens (no backend/data/query/mutation/logic change; every workflow + render condition preserved — adversarially verified, BEHAVIOR PRESERVED: yes). **Memories** (`app/(app)/memories/page.tsx`, `return` JSX only + a new module-scope `FeedSkeleton`): retired all off-brand chrome — the `bg-black` search button, the `bg-yellow-50` workspace banner, and the `text-gray` "Loading…/Updating…/Searching…" text. New serif header, a single **brand search field** (leading magnifier icon, sage focus ring, a live **clear ×** button; live-debounced + Enter search unchanged; `text-base`/16px to avoid iOS focus-zoom), calm reduced-motion-safe **skeleton** loading, an `aria-live` status region, and an explicit `htmlFor` label. `components/memories/MemorySection.tsx` (sticky section headers) + `components/MemoryCard.tsx` rebranded to brand tokens (no raw gray/stone), cleaner memory-date row (`CalendarClock` icon + `sr-only` "Memory date:"), sage focus ring; sticky offsets unchanged. **Home** (`app/(app)/home/page.tsx`): serif h1, ≥44px touch targets + sage focus rings on all three CTAs, calmer `max-w-2xl` centered column; new `app/(app)/home/loading.tsx` skeleton. **The React-Query feed (paginated→flat aggregation) + optimistic create/edit/delete + `handleSearch` debounce + sort/grouping are byte-unchanged** (verified: data-layer diff empty). Validation: tsc clean · lint 0 errors (2 pre-existing) · build ✓ · independent adversarial review with its 2 findings fixed (iOS zoom-on-focus → `text-base`; MemoryCard SR date context → `sr-only` label). **Polaris roadmap remaining:** People, Insights, Search, Timeline, Library, Settings, reminders-presentation. Authoritative note in `CLAUDE.md`. iOS `project.pbxproj` build-bump left UNSTAGED.

- **Project Polaris — Pass 1: Dashboard + shell UX refactor (product, 2026-07-08, NOT pushed).** First increment of the app-wide "Apple-quality, calmer, lower-cognitive-load" UX program — **presentation ONLY** (no backend/auth/schema/API/business-logic change; every feature preserved). **(1) Dashboard IA** (`app/(app)/dashboard/page.tsx`): the ~20 stacked widgets were regrouped into a calm progression — Greeting → Attention alerts → **Today** (reminder focus + Remy summary + date nudges) → **Jump back in** (quick-resume + create) → **People** (workspace summary + family) → **Insights** (a summary tile → `/insights` + the heavy Remy narrative layers) → **Account & storage**; the analytics-heavy layers collapse into a new zero-JS, keyboard-accessible native-`<details>` `CollapsibleSection` (children stay MOUNTED when collapsed → telemetry still fires). The **async data body is byte-unchanged**; only the `return` JSX + 2 imports + the local `CollapsibleSection` were touched, and **every original component invocation is kept with identical props/conditionals** (verified: data-layer diff empty; `PendingInvites` still null-on-empty). **(2) FAB → Remy** (operator-approved central interaction point): the green "+" is replaced by the **Remy avatar** (`<Remy state="welcome">`); tapping opens a **calm, portaled bottom-sheet** (`components/navigation/RemyActionButton.tsx`) — Ask Remy (`/remy`) / Add a memory (`/memories/new`) / Add a reminder (`/reminders`), **existing routes only, no deferred AI**. Sheet = `createPortal(document.body)` (REQUIRED — host nav has `backdrop-blur`), focus-trapped (`useFocusTrap`), scroll-locked, Escape-close, `context` query-param threaded. `FloatingCompanionButton` gained `variant "solid"|"nest"` (default solid unchanged) so the purple avatar reads on a clean pedestal. **Validation:** tsc clean · lint 0 errors (2 pre-existing warnings) · build ✓ · independent adversarial review. **Operator scope:** flagship-first then page-by-page; reminders page = presentation-only in a LATER pass (frozen logic untouched). **Next Polaris passes:** Home, Memories, People, Insights, Search, Timeline, Library, Settings, reminders-presentation. iOS `project.pbxproj` build-number bump left UNSTAGED (operator artifact). Authoritative decision recorded in `CLAUDE.md`.

- **Ask Remy — MILESTONE B: MEMORY INTELLIGENCE GRAPH (product, 2026-07-04, NOT pushed).** Taught Remy how memories CONNECT — a **deterministic** graph over people/memories/themes/time — by EXTENDING the existing Ask intelligence (NO new AI/embeddings/search/DB engine, NO hallucination, grounding prompt + retrieval untouched). Only **4 Ask-Remy files**. **`lib/remy/relationship-intelligence.ts` (extended, the existing people graph):** the dataset now also fetches `ai_category`/`ai_tags`; new PURE derivations `personMemoryMetas`, `topThemesOf` (recurring themes), `decadeCountOf` (life periods), `connectMemories` (shared memories among people); `buildPersonRelationshipContext` **rewritten to ONE dataset fetch** (was 3 — a perf win) + now returns `topThemes` + `periodCount`; `formatPersonRelationshipFacts` enriched (themes + decades); new `isConnectQuery` + `answerConnectPeople` (deterministic "which memories connect Mary and John", prefers all-co-appear then any-two). **New `lib/remy/memory-graph.ts` (aggregate temporal/theme, complements the people graph):** one owner+workspace-scoped `getMemoryStats` read + PURE `themeRanking`, `yearlyActivity`, `busiestYears`, `monthlyActivity`, `memoriesAround` (event/season → month + keyword), `buildMemoryGraphSummary`; `detectMemoryGraphIntent` (`themes` | `busiest-years` | `around`) + deterministic `answerMemoryGraph`. **`ask-action.ts` (orchestration extended):** routes, in order, relationship-aggregate → **memory-graph aggregate** (themes/busiest/around, deterministic, no LLM) → person-aware retrieval → **people-connection** (≥2 resolved people + connect keyword, deterministic) → single-person **graph enrichment** (now for ANY single matched person, not just "relationship"-worded — so **"Tell me about Mary"** is now rich: appears in N, span, recurring themes, decades, co-people); `AskAnswer` extended **additively** with optional `graph` (person/connection/aggregate summary). **`RemyAsk.tsx`:** a `GraphCard` renders the person/connection/aggregate summary (semantic `<dl>`, theme chips, busiest-years), replacing the generic facts strip when a graph is present; Related memories + follow-up chips + `AIDisclaimer` + all a11y preserved. **Answers now handled deterministically & grounded:** who I mention most, who appears together most, which memories connect X & Y, what memories happened around Christmas, what years were busiest, what themes appear most, tell me about {person}. **Isolation:** every new query owner + workspace scoped. tsc clean, lint **0 new** (2 pre-existing), build **✓** (64/64). **Deferred (documented):** "show memories similar to this" (needs a current-memory context the Ask surface lacks — reuse semantic retrieval on a memory later); most-connected-memories + full seasonal/decade cards; exact relative-date phrasing ("last week"); aggregate stats read the most-recent **3000** memories (bounded, like the existing 5000 person-dataset cap — raise/paginate for very large accounts). **Adversarially reviewed (all PASS); fixed the flagged MAJOR (`memoriesAround` now reports the true total, not the capped 10) + minors (id tie-breaks, person-scoped-query defers to person-aware, honest 3+-people connect wording, no mismatched card on "around", graph card survives an LLM phrasing failure).** Operator iOS files left unstaged.
- **Ask Remy — MEMORY INTELLIGENCE: MEMORY↔MEMORY GRAPH + IMPORTANCE + CENTRALITY (product, 2026-07-07, NOT pushed).** Finished the memory-intelligence layer so Ask Remy answers how *memories themselves* connect — not just people/aggregate. **Deterministic, grounded, no new AI/embeddings/search/DB engine, grounding prompt + retrieval untouched.** **New `lib/remy/memory-connections.ts`** builds a memory↔memory connection graph by REUSING the two existing workspace+owner-scoped loaders (`getMemoryStats` + `getRelationshipDataset`) — one fetch of each, then PURE inverted-index derivations (person/tag/category/year/**decade** → memories) so it is **bounded, never N²** (`COMMON_BUCKET_CAP=150` skips mega-common attributes; every signal — including decade — respects the cap). Exposes `scoredConnections`/`relatedMemories` (per-memory related set + WHY + score: W_PERSON 3 · W_TAG 1.5 · W_CATEGORY/YEAR 1 · W_DECADE 0.5), and `importanceRanking` → **memory importance** (0.5·degree + 0.3·people + 0.2·5-yr-recency) + **centrality** (`hub`/`bridge`/`connected`/`isolated`). Importance uses a single UNSORTED connection core (skips N unused per-node sorts). **`detectMemoryIntelIntent` + `answerMemoryIntel`** answer, deterministically: *most important / key memories*, *most connected (hub) memories*, *standalone (isolated) memories*. **`ask-action.ts`**: new memory-intel branch — **0-people-gated, after person-aware + connect-people + aggregate-memory-graph, before the LLM** (so it never hijacks those); `AskGraph` extended **additively** with an **`importance`** kind. **`RemyAsk.tsx`**: an importance `GraphCard` whose ranked rows ARE the clickable `/memories/[id]` links (so the intel result is one surface — no duplicate RelatedMemories list). **Isolation:** every read owner+workspace scoped; the new global ranking is now **fully deterministic** — added stable `.order("id")` tie-breaks before the caps in **`getMemoryStats`** and **`getRelationshipDataset`** (people/links/memories) so identical calls build identical graphs. **Relevance guard:** intent regexes require the centrality adjective to DIRECTLY qualify "memor" (+ reverse forms) AND a bare-query check (`isBareCentralityQuery`) — a constrained superlative ("most meaningful memories **from my wedding**", "important memories **in 2020**") DEFERS to the grounded LLM path rather than silently returning the global ranking; apostrophes normalized. **Reuse honesty (documented):** memory **clusters / life-chapters / storylines / collections / observations / connections ALREADY EXIST** as `userId`-scoped *dashboard* systems (`lib/remy/{connections,life-chapters,collections,observations,story}.ts`) — they were **NOT** rewired into workspace-scoped Ask because they lack a `memoryProfileId` filter and would **leak across workspaces**; the genuine gap built here is the workspace-scoped memory↔memory graph + importance/centrality. tsc clean, lint **0 new** (2 pre-existing), build **✓** (64/64). **Adversarially reviewed via multi-agent Workflow (4 rounds, 6 lenses: determinism/hallucination · graph-correctness · workspace-scoping · Ask-integration · a11y · performance) → final 0 BLOCKER / 0 MAJOR / 0 confirmed-serious.** Fixed en route: MAJOR regex over-match ("key people in my memories" hijack) → direct-adjacency + bare-query guard; MINORs — tag/person double-count (dedupe at build), decade signal bypassing the cap (byDecade index + same-year exclusion), hub card ordered by score not degree (true degree ranking), isolated/important lead overclaims (honest wording + heading "Your key memories"), cross-call non-determinism at the reused caps (secondary sort), apostrophe-split defer, reverse-form importance coverage; tz-safe year parse. **Deferred (documented):** "memories similar to THIS one" (needs a current-memory anchor the Ask surface lacks — reuse `relatedMemories` from a memory-detail context later); wiring the `userId`-scoped dashboard cluster/chapter/storyline systems into Ask (needs a workspace-scoping pass first); promoting all Ask card labels from `<p>` to semantic headings (a pre-existing component-wide pattern, uniform by design — out of scope for this card alone). Operator iOS files left unstaged.
- **Reminders — SPRINT 1: confirmed production-defect fixes (product, 2026-07-07, NOT pushed).** Resolved the four CONFIRMED defects from the Reminder Verification Sprint (which superseded the earlier "no active defects" claim — that only covered on-device *delivery*, validated on TestFlight Build 8; these gaps sit outside it). Scope-limited bug-fix only; **the frozen native scheduler is UNTOUCHED** (`lib/native-reminders.ts`, `NativeReminderSync.tsx`, `AppDelegate.swift` unmodified — the new beacon only READS `LocalNotifications.getPending()` + imports `isNativeReminderTarget`). Every schema-dependent change is **probe-gated → inert until the operator applies its migration** (zero behavior change on deploy). **(1) Per-occurrence recurring completion** — completing a recurring reminder no longer sets `completed=true` (which permanently ended the series on BOTH the native reconcile and the cron); it advances `remind_at` to the next FUTURE occurrence via the new shared **`lib/reminders/recurrence.ts`** (`nextOccurrence` extracted verbatim from the cron — one recurrence engine, no duplicate; `nextOccurrenceAfter` for the completion path). The routine lives on; the button now reads **"Done for today"** for recurring. Delete still ends a series. **(2) Processing-lock lease** — the cron `processing=true` lock could stick forever after a crash/timeout/deploy → permanent silent miss. New **`processing_at`** column (`20260707120000_reminder_processing_lease.sql`) + a race-safe reclaim in the cron (`PROCESSING_LEASE_MS=5min`; due-query + lock `.or(processing.eq.false,processing_at.lt.<stale>,processing_at.is.null)` — the `is.null` branch also recovers rows already stuck BEFORE the migration). **(3) Authorization parity** — `toggleReminderComplete`/`deleteReminder` now do an authoritative row fetch + `userCanAccessProfile` check (mirrors create + the `[id]` route) and scope writes by the reminder's OWN `memory_profile_id`/`user_id`, not the client-settable active-profile cookie. **(4) Native iOS duplicate prevention** — the device (via new **`<NativeReminderBeacon>`** → **`POST /api/reminders/native-active`** → **`reminder_local_confirmations`** table, `20260707130000`) reports which reminders have a PENDING local; the cron SKIPS the redundant OneSignal push for a reminder with a fresh, **user_id-matched** confirmation but STILL runs lifecycle bookkeeping (so the DB stays correct). **Fails TOWARD delivery** — any missing/TTL-stale (`2d`)/foreign confirmation falls back to the push (never a silent miss); composite PK `(reminder_id,user_id)` so multiple caregivers each confirm; confirmations cleared on complete/reopen/delete so a reopened one-time can't be suppressed by a stale row; batch-scoped load (no PostgREST truncation); RLS + user_id match block griefing. **Validation:** tsc clean, lint **0 new** (2 pre-existing), build **✓** (65/65). **Adversarially reviewed (independent subagent):** all 7 invariants held (backward-compat, lease race-safety, PostgREST `.or()` ISO parsing, per-occurrence correctness, authz, fail-toward-delivery, frozen-scheduler-untouched); its 4 substantive findings were FIXED (pre-migration NULL-lock reclaim, multi-caregiver composite key, reopen silent-miss cleanup, unbounded confirmation load). **OPERATOR STEPS (required to activate items 2 & 4):** apply `20260707120000_reminder_processing_lease.sql` and `20260707130000_reminder_local_confirmations.sql` in the Supabase SQL editor; then a native iOS build for the beacon (item 4). **Residual (documented, needs on-device verification):** the native dedup can't guarantee single-delivery with 100% certainty without an iOS Notification Service Extension (operator/native) — if a device stops confirming (permission revoked mid-TTL, iOS purges a schedule), the cron resumes pushing (safe duplicate, never a miss); item 1's "Done for today" on a still-future occurrence advances one cadence (cosmetic; series preserved); the cron recurring reschedule's DST/month-end drift (B-3) is a SEPARATE unfixed defect, out of this sprint's scope. Operator iOS files left unstaged.
- **Production Sprint 2 — authorization + webhook reliability (security, 2026-07-07, NOT pushed).** Fixed the TWO HIGH findings the production audit confirmed. **(1) Stripe webhook reliability** (`app/api/stripe/webhook/route.ts`): every event handler previously LOGGED a failed `profiles` write but still returned HTTP 200, so Stripe never retried → premium/subscription/entitlement/quota could permanently desync from payment. Now a `writeFailed` flag is set inside each write-error branch (checkout.session.completed · subscription.created · subscription.deleted · subscription.updated · invoice.payment_failed) and the handler returns **HTTP 500** at the end if any required write failed, so Stripe **retries the whole event**. Signature verification, event handling, and service-role scoping are unchanged; retries are safe because every write is idempotent (`.update().eq(id|customer|subscription)`). A `!data` "no matching profile row" (missing metadata / already in that state) is intentionally NOT a failure — it stays a 200 ack so a permanently-unmatchable event can't loop forever. **(2) Memory-create authorization parity** (`app/api/memories/create/route.ts`): the route inserted `memory_profile_id = activeProfileId` (from the **client-settable** active-profile cookie) with NO access check — unlike `createReminder`. Added, right after resolving `activeProfileId` and BEFORE any quota/storage/insert mutation: `if (activeProfileId && !(await userCanAccessProfile(user.id, activeProfileId))) → 403`. My Nest (null profile) is unaffected; application-layer guard that holds even if RLS also blocks it. **Validation:** tsc clean, lint 0 new (2 pre-existing), build ✓ (65/65). **Adversarially reviewed (independent subagent):** both fixes SOUND — all 5 webhook handlers capture write failures (incl. the two-attempt deleted/updated fallbacks, keyed off the FINAL error so no false-500), no non-retryable condition loops, idempotent retries; memory authz is correctly placed before all mutations, allows My Nest, fail-closed, no bypass. **One LOW residual (documented, NOT fixed — pre-existing, out of scope):** in the deleted/updated two-attempt handlers, a *transient* error on the customer-id attempt followed by a *no-row* subscription-id fallback masks the transient error → a false 200 (rare downgrade loss); the fix neither introduces nor worsens it, and the naive "capture any error" alternative would reintroduce false-500s. **Remaining confirmed audit issues (unfixed, out of THIS sprint's scope):** the `memories` INSERT RLS still needs manual verification in the Supabase SQL editor (H-2 exploitability is RLS-dependent); Stripe `cancel_at_period_end` not surfaced (M); attachment-drop on null Storage size (M); cron DST/month-end drift (M); and ~40% of audit domains (full security sweep, a11y, performance, PWA/offline, deep auth) were not completed (session-limit interruption) and still need the deferred pass. **Not production-ready.** Operator iOS files left unstaged.
- **Ask Remy — PHASE 4.1 CONVERSATIONAL INTELLIGENCE (product, 2026-07-04, NOT pushed).** Made Ask Remy feel like a memory companion, not a search box — by **extending the existing AI intelligence layer** (NO duplicate systems, NO new AI/DB calls, NO companion-platform / frozen-system changes). Only **4 Ask-Remy files** touched. **New `lib/remy/ask-insights.ts`** (pure/deterministic): derives, from the memories the Retrieval Engine ALREADY fetched, a **facts** summary (people from the person graph, recurring **themes** from `ai_category`+`ai_tags`, calendar **date span**), the **related memories** to open, and grounded **follow-up suggestions** (each validated via `extractAskQuery` so no chip is ever a dead end) — zero fabrication (only what the memories record), zero extra AI/DB. **`app/(app)/remy/ask-action.ts`**: `AskAnswer` extended **additively** (optional `facts`/`related`/`followUps`), populated from `records`; the server-action signature + all grounding invariants (retrieve-before-generate, no-AI-on-empty, LLM sees only retrieved memories, non-clinical) are unchanged; insights attach even when the LLM answer fails (graceful degradation). **`lib/remy/ask-intent.ts`**: new `resolveContextualFollowUp` so a contextual follow-up ("**What about last year?**" after "Tell me about Mary") **keeps the prior subject** (Mary) and merges the qualifier (last year → this year−1) — conservative (only clear "what about/how about/what of" leads with an anchor; ordinary questions unaffected). **`components/remy/RemyAsk.tsx`**: rich conversational answer — a compact **facts strip** (`<dl>`), **Related memories** links (→ `/memories/[id]`), and tappable **follow-up chips** (each phrasing re-parses to a real query); refactored to a shared `runTurn` (chips + submit); a11y preserved (aria-live answer, aria-label chip group, keyboard buttons/links, `AIDisclaimer` footnote, role=status). **Personality:** warm/gentle/non-clinical, not chatty (facts compact, ≤4 follow-ups); **no emotional-patterns/mood-scoring** added (avoids clinical framing). **Performance:** no new AI or DB calls — facts/follow-ups are pure functions of the already-retrieved records. tsc clean, lint **0 new** (2 pre-existing), build **✓** (64/64). **Deferred (documented):** data-derived facts that need extra queries (exact "18 memories", places — no reliable place field; birthdays/reminders) → a future retrieval could supply them; the LLM answer itself is still free-text (structured sections are the deterministic strip around it). Operator iOS files left unstaged.
- **Remy PLATFORM v2 — PHASE 3 LIVING COMPANION (behavior + reactions, 2026-07-04, NOT pushed).** Made Remy feel alive — **entirely inside the platform** (no feature/bus/API/frozen changes; event-bus/events/public-API byte-unchanged). **6 platform files** touched. **Reactions:** the existing events now produce emotional responses with **motion + a speech bubble** — e.g. `memory.created`→happy/bounce/"Memory safely preserved.", `memory.saved`→happy/"I've updated that.", `memory.deleted`→calm/"Removed.", `failure`→confused/wobble/apologetic, `offline`→concerned/wobble, `online`→calm, `search.started`→thinking/tilt. **New `lib/remy/core/speech.ts`** — per-event + per-context lines + session-adaptive lines + greetings + milestone lines, non-repeating (avoids the last line), non-clinical. **`lib/remy/core/brain.ts`** — additive **session memory** (created/saved/deleted/searches counters + `transientEvent`) drives adaptive personality ("We're preserving lots today", "Looking for something specific?"); the **first memory of a session** escalates to a celebration. **New `components/remy/motion.module.css`** — the CSS animation backend: continuous **breathe** + **float**, one-shot **bounce/celebrate/wobble/tilt/pulse/appear**, resting **sleep**, natural cubic-bezier easing, **all disabled under `prefers-reduced-motion`**. **`components/remy/Remy.tsx`** — optional `emotion`/`float`/`reactionKey` props map feeling→motion (idle sprites just breathe; in-place `<RemyStage>` mascots now gently breathe too — no regression). **`components/remy/companion/RemyProvider.tsx`** — the living orchestration: a **session greeting** (once, ~1.4s after load), **rest→sleep** after 90s inactivity, and **rate-limited speech** (≤ once / 6s, keyed to its reaction so no stale lines). **`FloatingCompanionLayer.tsx`** — renders the speech bubble (`aria-live`) + drives the renderer motion. **Safety (§8):** the overlay is `pointer-events-none` (never blocks scroll/forms/buttons — only the small corner badge is tappable; the bubble is non-interactive), z-45 below modals, safe-area-anchored, rate-limited, no modals. tsc clean, lint **0 new** (2 pre-existing), build **✓** (64/64). **Deferred (need feature data or frozen edits, documented with hooks):** data-derived greetings (weekly counts, birthdays) + total-count milestones (10/25/100 memories, first family member, 365-day) need app data via an event payload; conversation-awareness needs the frozen AI feature to emit `conversation.started/ended` (platform reaction is ready); frame-based blink/wing-flap needs animated assets (Rive/Lottie behind the ready seam). Operator iOS files left unstaged.
- **Remy PLATFORM v2 — APPLICATION EVENT WIRING (integration, 2026-07-04, NOT pushed).** Wired the app's real workflows into the (frozen) Remy Event Bus so the Brain receives meaningful semantic events — **integration only, zero architecture/vocabulary changes, zero platform-core edits, zero frozen-system edits.** All events use the EXISTING vocabulary (no new events/contexts introduced). **Wired (client-side, additive, no business-logic change):** `app/(app)/memories/page.tsx` — `memory.created` (create onSuccess), `memory.saved` (update onSuccess), `memory.deleted` (new delete onSuccess), `failure` (all 3 mutation onError), `search.started`/`search.finished` (live search handler); `components/CreateMemoryForm.tsx` (dashboard + /memories/new standalone create) — `memory.created` on save / `failure` on error; `components/search/SearchView.tsx` (global search) — `search.started`/`search.finished`; `app/(app)/timeline/page.tsx` — **`<RemyStage context="timeline.empty">`** empty state; **new `components/RemyConnectivityBridge.tsx`** (mounted once in `(app)/layout.tsx`) — `offline`/`online` from browser events. Empty-state contexts now cover the operator's four examples: memories.empty + people.empty (already) + timeline.empty + search.empty (memories no-results already). **All feature code imports ONLY `@/lib/remy`** (`Remy.emit` / `RemyStage`) — verified: no feature reaches a companion internal; no `expression=`/`state=`/`showRemy`/`setMood` hardcoding anywhere. **Deferred (frozen — reported with ready emit lines, NOT edited):** reminders (create/complete/dismiss), auth (login/logout), billing/subscription changes, notifications/OneSignal receipt, AI conversation start/end (`RemyAsk`/`RemyConversation`) — adding emits there would modify frozen systems; also the secondary exploration empty states (connections/chapters/collections/library) which would need new contexts (deferred per "reuse existing vocabulary / don't invent unnecessary events"). **Behavior note:** the floating presence now reacts to these events (brief happy/thinking/calm/concerned) — the intended companion behavior; frequency/visibility is a one-file policy tune if desired. tsc clean, lint **0 new** (2 pre-existing), build **✓** (64/64). Files: memories page, CreateMemoryForm, SearchView, timeline page, layout, + new ConnectivityBridge. Operator iOS files left unstaged.
- **Remy PLATFORM v2 — full platform-service pipeline behind one public API (2026-07-04, NOT pushed). SUPERSEDES the v1 event-platform entry below.** Elevated Remy from a UI platform into a first-class platform SERVICE (like auth/analytics/theme/router). Pipeline: `Public API → Event Bus → Brain → Emotion Engine → Policy Engine → Animation/Voice Engines → Renderer`. **New pure-TS core `lib/remy/core/*`** (no React/DOM → portable to native hosts): `event-bus.ts` (framework-agnostic pub/sub singleton — the single entry), `events.ts` (`RemyEventName`/`RemyContextKey` vocabulary + shared `CONTEXT_PRIORITY`), `dispatch.ts` (emit helpers), `brain.ts` (`RemyBrain` — interprets events → semantic state: context stack + transient moment feeling; built for future memory/relationship/trust/personality expansion), `emotion.ts`+`emotion-engine.ts` (event/context → feeling), `policy-engine.ts` (feeling+source → `RemyPresentation` — the SOLE presentation authority; `resolvePresentation` for the whole state), `presentation.ts` (the expression vocabulary — now the single source, aliased by the renderer), `animation-engine.ts` (dedicated cue engine over the kept `AnimationController` seam), `voice-engine.ts` (interface + disabled no-op — ARCHITECTURE ONLY, no TTS). **`RemyProvider` rewritten as a THIN React adapter** over the core (bus→brain→React state→policy→engines); keeps split state/actions contexts + stable `children` (perf contract intact). **`RemyStage` now takes `context=` and emits it to the bus** + renders the policy-chosen expression via the one `<Remy>` renderer. **`FloatingCompanionLayer`** renders the resolved presentation (dismissable). **THE single public API `lib/remy/index.ts`** exports `Remy` (facade: `emit`/`enter`/`exit`/`on`) + `RemyProvider`/`RemyFloatingPresence`/`RemyStage`/hooks/types — **features import ONLY `@/lib/remy`**. **Rewired** the 3 pages + `(app)/layout.tsx` to `@/lib/remy` (layout no longer imports internals). **Deleted the v1 layer** (`lib/remy/platform/{scenes,policy}.ts`, `components/remy/platform/index.ts`, and the superseded `lib/remy/companion/state.ts`). **Verified exactly ONE** of: public API, renderer, provider, asset registry, event bus, brain; and **zero** feature imports of any companion-platform internal (error boundaries keep raw `<Remy state="confused">` — the documented resilience exception; the many `@/components/remy/*` imports elsewhere are the SEPARATE AI-intelligence UI + frozen avatar sprite, untouched). Frozen systems untouched; operator iOS files left unstaged. tsc clean, lint **0 new** (2 pre-existing), build **✓** (64/64). **New definitive doc `docs/architecture/REMY_PLATFORM_V2.md`** (layer diagram, event/state flow, every engine, public API, renderer, asset ownership, future AI/native/wearable/desktop, extension guidelines, contributor rules). CLAUDE.md v2 note supersedes v1. **Future-proof:** add a moment = event+feeling+look mapping + emit; add animation/voice = backend behind a `create…` seam; go native = new adapter+renderer over the same core — features never change. **Tech debt:** events are defined but few are emitted yet from feature flows (wiring `Remy.emit("memory.saved")` etc. into save/search/sync/offline handlers touches business logic — deferred, out of this architectural scope); Voice Engine is interface-only; art still needs transparent backgrounds (see below).
- **Remy PLATFORM ARCHITECTURE — event-driven, decoupled from pages (2026-07-04, NOT pushed). (v1 — superseded by the v2 entry above.)** Turned Remy from page-coupled rendering into an application-wide **platform** (`Application → Remy Platform → UI`, like theme/toast/router). **The UI now emits SEMANTIC scenes/signals; the platform decides expression/visibility/animation** (future voice/gesture/emotion). **New semantic layer:** `lib/remy/platform/scenes.ts` (the vocabulary — `RemyScene` sticky contexts + `RemySignal` transient moments + `SCENE_PRIORITY`) and `lib/remy/platform/policy.ts` (the SINGLE map semantics→`RemyPresentation` {expression, visible, durationMs}; swap it → re-skins every Remy, zero page edits). **Runtime:** evolved `components/remy/companion/RemyProvider.tsx` into the platform brain — scene stack (immutable `Map` in state) + auto-expiring signal + manual open/close, resolves the active presentation via policy; **kept the split state/actions contexts + stable `children` perf contract** (Remy activity never re-renders the app tree). **New public API barrel `@/components/remy/platform`:** `<RemyStage scene="…"/>` (in-context surface — emits the scene AND renders the policy-chosen expression via the single `<Remy>` renderer), `useRemyScene`, `useRemySignal`, `useRemyController`. **Refactored** `FloatingCompanionLayer` to render the platform presentation (dropped the local `STATE_TO_VARIANT`). **Decoupled the 3 pages** — home/memories/profiles `<Remy state="welcome|confused">` → `<RemyStage scene="welcome|empty.memories|search.empty|empty.people">` (pages no longer choose expressions). **Kept raw `<Remy>` in both error boundaries** (`app/error.tsx`, `app/(app)/error.tsx`) as a **documented resilience exception** — the platform provider may be the thing that crashed / is unmounted at the root. Single renderer (`Remy.tsx`) + single asset registry unchanged. **Frozen systems untouched** (no reminders/auth/billing/AI/notifications/db); operator iOS files left unstaged. tsc clean, lint **0 new** (2 pre-existing), build **✓** (64/64). **New permanent doc `docs/architecture/REMY_PLATFORM_ARCHITECTURE.md`** (single source of truth: layers/responsibilities/event+state flow/rendering+asset pipeline/component hierarchy/future animation+voice+AI+multi-platform/extension guidelines/design principles/subsystem file map). CLAUDE.md gained the authoritative platform note. **Future-proof:** add a moment = edit scenes.ts + policy.ts + emit; add animation/voice/emotion = additive `RemyPresentation` field + a controller behind an interface; swap render tech = edit only `<Remy>` — pages never change. **Remaining tech debt:** signals are built but not yet emitted from feature flows (deferred — wiring `success`/`memoryFound` into memory-save touches business logic, out of this architectural scope); the floating presence has no user trigger yet (dormant by design); transparent-background art still pending (see below).
- **Remy artwork ENGINEERING INTEGRATION — centralized `<Remy>` component + presentational wiring (2026-07-01, NOT pushed).** The Remy asset library is complete (23 real approved PNGs; the operator dropped the final two — `nest_open` + `shadow` — so **0 placeholders remain**; registry flipped both `kind:"placeholder"`→`"image"`). **New `components/remy/Remy.tsx`** — the SINGLE, centralized render path: `<Remy state="thinking" size={120} fit="contain" />`. It maps 17 friendly variants → registry KEY → `src` (registry stays the **sole** owner of paths; **no duplicated image logic** anywhere), uses `next/image` `fill` + `object-contain|cover` in a fixed square box (aspect-safe for the 1536×1024 art), is **server-component-safe** (no `"use client"`), and is **animation-ready by structure** (a `.remy` root + `.remy__sprite` inner layer + `data-remy-variant`/`data-remy-kind` hooks → float/breathe/blink/transition or a Rive/Lottie swap plug in later behind the existing `AnimationController` seam with **zero API/consumer change**; no animation implemented yet). **Integrated (presentational only, no logic touched):** `FloatingCompanionLayer` (replaced the text "Remy" placeholder → circular `<Remy fit="cover">`, state-mapped via `STATE_TO_VARIANT`), both error boundaries `app/error.tsx` + `app/(app)/error.tsx` (`confused`), and the empty states of `home` (first-run `welcome`), `memories` (empty `welcome` + no-results `confused`), `profiles` (empty `welcome`). **Frozen systems correctly SKIPPED** (audit found 152 candidate placements / 115 "safe"): NO reminders/auth/billing/Stripe/AI(`/remy`,`RemyAsk`,insights,`ask-action`)/notifications/database/business-logic files touched; the **frozen avatar sprite** (`components/remy/avatar/*`, `public/remy/*`) untouched; the 4 operator iOS files left unstaged. tsc clean, lint **0 new** (2 pre-existing), build **✓** (64/64), adversarially verified. **⚠️ Pre-App-Store flag:** every export ships with an **opaque gray-gradient background** (not transparency) — Remy renders as a soft rectangle/badge rather than a cut-out; re-export with alpha for clean compositing (esp. `shadow`, which needs transparency to work as a ground shadow). **Recommended follow-ups (not done, presentational):** empty/loading states for timeline/search/connections/chapters, and (behind the AnimationController) idle float/breathe once art is transparent. See `components/remy/Remy.tsx`.
- **Remy asset pipeline — FLATTENED to a single folder (2026-07-01, NOT pushed). SUPERSEDES the 2026-06-28 master/production/archive structure below.** Operator re-prioritized: the 3-folder pipeline was over-complicated → **every Remy/Nest asset now lives DIRECTLY in `public/assets/remy/`** (no sub-folders). The operator dropped a **complete set of 22 unique real approved artwork PNGs** (2.4–3.2 MB each) into `public/assets/remy/master/`; this task **moved all 22 approved files into the flat folder**, **created 2 transparent placeholders** (`nest_open.png`, `shadow.png` — no approved art yet), and **deleted** `production/`, `archive/`, and `master/` plus the obsolete `remy_wave.png` (real art, but not in the operator's new target list — the state set dropped `wave` and added surprised/searching/memory_found/reminding/encouraging/welcome/goodbye/confused/wink/success). Final folder = **24 PNGs** (`remy_master_v1.png` + 21 real art + 2 placeholders) + README, **0 sub-dirs**. `remy_master_v1.png` (md5 unchanged `9a722e…`) stays the **read-only canonical reference**, **not** registered as an app asset. **Asset Registry** (`lib/remy/companion/asset-registry.ts`) rewritten: `BASE` `"/assets/remy/production"` → **`"/assets/remy"`**, expanded to **23 keys** (17 remy states + golden_feather + 3 nests + speech_bubble + shadow), all `kind:"image"` **except** `nestOpen`+`shadow` (`placeholder`). Verified: registry is the **sole** path owner; **23/23 refs resolve**; **ZERO** references to `/master/`·`/archive/`·`/production/` remain anywhere; tsc clean, lint **0 new** (2 pre-existing), build **✓** (64/64). README + CLAUDE.md note rewritten for the flat layout. **⚠️ Honest flag for operator:** the approved exports have a **non-transparent gray-gradient background** (the master sheet spec said transparent) — moved as-is per "these are the approved production artwork"; re-export with alpha if transparency is wanted. **Operator iOS working-tree changes left unstaged** (unrelated). *(Historical note — the entry below describes the now-removed 3-folder structure.)*
- **Remy production asset pipeline + repo reorganization (Phase 2) (2026-06-28, NOT pushed).** Establishes **Remy Master v1.0** as the canonical character reference and reorganizes `public/assets/remy/` into a clean **master / production / archive** structure; **no app behavior change** (nothing renders the registry assets yet — the Floating layer still draws its neutral placeholder), and reminders/notifications/auth/payments **untouched**. **`public/assets/remy/master/remy_master_v1.png`** (operator-dropped, 1.72 MB) = the approved turnaround+expressions+palette sheet → **read-only, never modified, never wired into the app**. **`public/assets/remy/production/`** (NEW) = the **only** location the app reads — 14 named assets: **2 real art** (`remy_idle.png`, `remy_thinking.png`, operator-dropped) + **12 transparent placeholders**. **`public/assets/remy/archive/`** (NEW) = a faithful snapshot of the pre-reorg flat layout (move, **nothing deleted**); includes the operator's `remy_listening.png` which was a **byte-exact duplicate of `remy_thinking.png`** (wrong pose) — so production listening stays a **placeholder** until its correct **wing-to-ear** export lands. Recovered idle from the hidden `.remy_idle.png` dotfile → `production/remy_idle.png`. **Asset Registry** (`lib/remy/companion/asset-registry.ts`): `BASE` repointed `"/assets/remy"` → **`"/assets/remy/production"`**, `remyIdle`+`remyThinking` flipped `kind:"placeholder"`→**`"image"`**, doc-comment rewritten (master/archive never referenced). Verified: the registry is the **sole** path owner (repo grep — no other file references `assets/remy`), all **14 refs resolve** to real production files, **0 broken refs**. README rewritten as the 3-folder guide. tsc clean, lint **0 new** (2 pre-existing warnings), build **✓** (64/64). **Operator/designer follow-up:** drop the remaining **12** production exports (incl. the correct `remy_listening`) into `production/` using the exact filenames + flip each registry `kind` to `"image"`; every export must match `master/remy_master_v1.png` (immutable brand). **Operator iOS working-tree changes (project.pbxproj/Info.plist/Podfile/Podfile.lock) left unstaged** — unrelated to this task.
- **Remy companion — PHASE 1 FOUNDATION (architecture only) (2026-06-28, NOT pushed).** Operator re-prioritized: build the plug-in framework for the Remy AI Memory Companion — **NO artwork, NO animations, NO AI, NO conversation** (CLAUDE.md deferral narrowed to exclude this foundation; the LIVE companion content stays post-launch-deferred). Reminder/notification system **untouched** (verified empty diffs). **Added** `lib/remy/companion/`: `state.ts` (the lifecycle **state machine** types: closed/opening/idle/listening/thinking/talking/celebrating/returning/hidden + future emotion/speech types), `asset-registry.ts` (central **key→asset** map, all `placeholder` — swap art here, never embed paths), `animation-controller.ts` (the `AnimationController` **interface** + dependency-free placeholder impl + `createRemyAnimationController()` seam — **no Rive/Lottie import**; Rive/Lottie/CSS/Framer plug in later with zero consumer change), `ai-hooks.ts` (`RemyAIHooks` **types only** — onListening/onThinking/onSpeaking/onMemoryCreated/onCelebration). `components/remy/companion/`: **`RemyProvider`** (split **state** vs **actions** contexts → `openRemy/closeRemy/toggleRemy/play` stable; `currentState/isVisible`; reduced-motion → controller), **`FloatingCompanionLayer`** (portaled-to-body overlay = Remy's home; `z-[45]` above content/nav, below modals; `pointer-events-none` layer + `auto` companion; safe-area `env(...)`; Escape/keyboard; placeholder only), **`FloatingCompanionButton`** (dual-mode: `href`=Link today / `onActivate`=button later — the Nest takeover seam). **Modified:** `components/navigation/MobileBottomNav.tsx` (center "+" refactored to `FloatingCompanionButton`, **byte-equivalent** href/aria/icon/haptic/styling → memory creation unchanged), `app/(app)/layout.tsx` (mounts `RemyProvider` wrapping the shell + `FloatingCompanionLayer`). **Performance:** `children` is a stable prop + split contexts → opening/closing Remy re-renders ONLY the provider + the layer, **never the app/nav/page** (adversarially verified). **SSR-safe** (`isVisible` false initial → layer null on server + first client render → no portal SSR / no hydration mismatch). Adversarially reviewed (4 agents, all **PASS**): perf, reminder/nav no-regression, a11y, architecture/Rive-readiness. tsc clean, lint **0 new** (2 pre-existing), build **✓** (64/64). **Phase 2 (still deferred):** real Rive/Lottie + artwork, emotional state transitions, conversation UI, AI connection.
- **RDAT follow-up — workspace switch (Build 11 still-stale) + safe-area overlay close (2026-06-24, NOT pushed).** **Reminder system untouched** (verified: empty diffs on `reminders/`, `NativeReminderSync`, `AppDelegate`, OneSignal). **Workspace switch (P0):** RDAT-002's `invalidateQueries(["active-profile"])` was insufficient on the native iOS WKWebView — it depended on **re-reading the just-set cookie**, which races WKWebView cookie propagation, so the client refetch returned the **previous** id and the feed stayed stale. **New approach — optimistic, cookie-read-free:** new **`lib/active-profile-cache.ts`** (`ACTIVE_PROFILE_QUERY_KEY` + `setActiveProfileCache`/`invalidateActiveProfileCache`); every switch entry point now **`setQueryData(["active-profile"], {activeProfileId: <known id>})`** with the id the client already has (`WorkspaceSelector` care=`profile.id`, `WorkspaceBanner`/`ProfileSwitcher` My Nest=`null`) + `router.refresh()` for chrome + invalidate-revert on failure. The memories feed keys on `ACTIVE_PROFILE_QUERY_KEY`. **Adversarial review caught a second leak:** the feed sent `?profileId=` only for Care; **My Nest sent bare `/api/memories` → fell back to the cookie → Care→My Nest still raced.** Fixed: the My Nest fetch now sends **`?workspaceType=my-nest`** so the GET scopes from the request param, not the cookie. All 3 directions (My Nest→Care, Care→My Nest, Care→Care) now use **explicit params → no cookie dependency → instant, no remount**. *(Residual, lower severity: `/api/memories/search` is deliberately cookie-authoritative — it ignores client workspace for security — so a just-switched search can briefly use the old scope while typing; not the memory-list bug.)* **Safe-area overlay close (P2):** new reusable **`components/ui/OverlayCloseButton.tsx`** anchored at `top-[max(0.75rem,env(safe-area-inset-top))] right-[max(0.75rem,env(safe-area-inset-right))]` (clears notch/Dynamic Island, portrait+landscape, native+web; `viewport-fit=cover` already set). `PhotoViewer` close → the component; the counter (`top-4`) and prev/next nav buttons (`left-2`/`right-2`) → safe-area-aware. Audit: only PhotoViewer collided — `MobileNavDrawer` already uses `pt-[max(1rem,env(safe-area-inset-top))]`; the create/edit/upgrade/storage modals are vertically centered. tsc clean, lint **0 new** (2 pre-existing), build **✓** (64/64). Reviewed (4 agents): RQ cache **correct**, all-3-directions fixed, safe-area **correct**, **no reminder regression**.
- **RDAT defect fixes — RDAT-001 (iOS video-capture crash) + RDAT-002 (workspace-switch stale feed) (2026-06-24, NOT pushed).** **RDAT-001:** added **`NSMicrophoneUsageDescription`** to `ios/App/App/Info.plist`. The capture path is the WebView `<input accept="image/*,video/*">` (no `@capacitor/camera`); switching the native camera to **Video mode** accesses the microphone for the audio track, and with no usage string iOS **force-terminated the app**. Fix is **+2/−0 additive** (`plutil -lint` OK); all four capture keys now present (Camera/Microphone/PhotoLibrary/PhotoLibraryAdd); **photo capture byte-identical/unaffected**. **NATIVE change → needs a fresh TestFlight build** (Info.plist lives in the binary, not the remote-URL web deploy). **RDAT-002:** the Memories feed loaded `activeProfileId` once via `useEffect([])`, so a workspace switch's `router.refresh()` updated the cookie + server chrome (label/banner) but **not** the client state → the `["memories", workspaceType, activeProfileId]` React Query key stayed frozen → **stale feed until remount**. Fix: **`app/(app)/memories/page.tsx`** now sources the active workspace from a React Query **`["active-profile"]`** query (replacing the one-time effect — same `undefined`-while-loading gate + null/id normalization), and **`components/navigation/WorkspaceSelector.tsx` `switchTo`** now **`invalidateQueries({queryKey:["active-profile"]})`** after the cookie-setting action (before `router.refresh()`) → the new id flows into the memories key → the **feed + on-page search re-scope immediately, no remount**. Adversarially reviewed (3 agents — all **CORRECT**): loading gate preserved (React Query v5 retains data across the invalidate refetch → no `undefined` flash, never shows another workspace's/user's data); optimistic create/edit/delete re-scope to the new workspace (no stale closure); **FROZEN reminders/notifications UNTOUCHED** (empty diffs on `reminders/`, `NativeReminderSync`, `AppDelegate`, OneSignal — reminders derive the profile **server-side**, already correct on switch). tsc clean, lint **0 new** (2 pre-existing warnings), build **✓** (64/64). **Forward note (non-blocking, out of scope):** the **My Nest** switch lives in `ProfileHub` and **navigates to `/home`**, so it's already correct via remount — only the in-place **care-profile** switch (the drawer) needed the fix.
- **Real Device Acceptance Testing (RDAT) plan — CREATED (2026-06-24, NOT pushed).** New `docs/PRODUCTION_ACCEPTANCE_TESTING.md` — the operational acceptance plan that converts the **86/100 code-confidence** score into a **runtime-verified** one. **80 test cases** (43 P0 / 25 P1 / 12 P2; 37 Blocker-severity) across **6 domains**: Media uploads, Reminders & notifications, Care profiles & workspace switching, Search, Mobile platforms (iPhone Safari/PWA/TestFlight + Android Chrome/Play), Offline/poor-network. Each case is **grounded in the real implementation** (quotes actual UI labels/states: "Add photos or videos", "Save Memory"→"Saving…", "Memory saved" toast, StorageFullModal, native-local-notification delivery, deferred enrichment timing) with preconditions, numbered steps, exact expected result, pass/fail line, severity, and target devices. Includes the 5 required cross-cutting sections: **§2 Pass/Fail criteria** (+ universal-FAIL conditions), **§3 Launch blockers** (blocking vs accepted-non-blocking — Android push is ⊘ BLOCKED pending FCM), **§4 score-update methodology** (priority-weighted RPR + P0-coverage gate + penalty model), **§5 release recommendation criteria** (Ready / Ready-with-monitoring / Not-ready thresholds), and **§6 execution tracker + sign-off**. Doc-only (no code). Authored via a 6-agent grounding workflow.
- **Direct-to-Storage uploads — IMPLEMENTED + adversarially hardened (2026-06-24, NOT pushed).** Media now uploads **client → Supabase Storage directly** (signed URLs); the API routes receive **JSON metadata only** → the ~4.5 MB Vercel function-body limit no longer applies, so **large videos/mixed-media succeed**. Flow: client → `POST /api/memories/upload-url` (auth + **server-authoritative quota pre-check** + **server-generated owner-scoped paths** `users/{userId}/...` + `createSignedUploadUrl`) → `uploadToSignedUrl` straight to Supabase → submit JSON to `/api/memories/create` (or PUT `/api/memories/[id]` with `newAttachments`). **Security model (server-authoritative):** paths are server-generated (no client choice); **quota is RE-VERIFIED at create/edit against the REAL object size** read from Storage (`getStorageObjectInfo`, never the client) — under-reporting at sign time is caught + 413 + orphan cleanup; **every attachment path is owner-scoped** (`isOwnedStoragePath`) on create AND edit (incl. the **kept** set — closes cross-user reference theft). New: `app/api/memories/upload-url/route.ts`, `lib/storage/object-info.ts`, `lib/memory-direct-upload.ts` (`uploadAttachmentsDirect` + `UploadQuotaError`). Edited: create + PUT routes (JSON direct path; legacy multipart kept **dormant for rollback**), `lib/memory-media.ts` (`handleMemoryMediaUpload` derives cover for the metadata-only path), and **all three live clients** — `CreateMemoryForm`, feed `createMutation` (now → `/api/memories/create` JSON; **fixed a pre-existing 405** where it posted to the unhandled `POST /api/memories`), feed `editMutation`. Diagnostics: `SIGNED_UPLOAD_REQUEST`/`_SUCCESS`, `DIRECT_UPLOAD_COMPLETE`, `MEMORY_CREATE_JSON`/`_SUCCESS`. **Adversarial review (6 skeptics) found + I FIXED 3 real bugs:** (1) 🔴 cross-user reference theft via un-scoped kept attachments on PUT → added a final owner-scope gate; (2) 🔴 cover wiped on no-new-file edits (clients stopped sending `coverImageUrl`) → else-branch recomputes cover from the kept set; (3) 🟠 feed `createMutation` still multipart → migrated. Re-verified all 3 **FIXED**; care-profile isolation **holds** (unchanged — `memory_profile_id` still from server cookie context). **Validation + hardening pass (2026-06-24):** a 2nd full adversarial sweep (6 vectors) — rendering / cover / enrichment / profile-scoping all **PASS** — found + **FIXED 5 more**: 🔴 **path-traversal owner-guard bypass** (`normalizeStoragePath` is now a positive `[A-Za-z0-9._-]`-segment allowlist that rejects `..`, backslash, empty **and `%2e%2e`/percent-encoded** segments — an attacker path could otherwise pass `startsWith` yet resolve to a victim object via the singular transform signer); 🟠 **server-side MIME re-validation on the direct create/edit paths** (`isAllowedAttachmentMime`, shared with the sign endpoint, **excludes `image/svg+xml`** incl. parameterized `;charset=` — stored-XSS hygiene); 🟡 **removed the client-size fallback** — an unverifiable real size now **fails closed** (drops the attachment) instead of trusting the client number. **⚠️ Residual (documented, NOT launch-blocking):** (1) concurrent-create **TOCTOU** quota race + (2) reused-path cross-memory **double-count** — both **pre-existing** (read-then-insert + per-`memory_id` ledger key); proper fix = a DB advisory-lock/`SECURITY DEFINER` RPC + per-object accounting (migration = operator). (3) **orphan** objects (uploaded, never attached) aren't ledger-counted — needs a sweeper cron. (4) **real stored content-type spoofing** — a client can upload svg/html bytes with a faked *declared* type; **LOW** (private bucket on the cross-origin `*.supabase.co` storage origin, `<img>`-embedded svg is inert, only direct signed-URL navigation executes — in the storage origin, not the app) → fix at serve (force `Content-Disposition: attachment` for non-rendered types / media proxy). tsc clean, lint **0 new** (2 pre-existing warnings), build **✓** (64/64). **Operator:** set the `memory-media` bucket object-size limit for video. See `docs/features/media-system.md`.
- **Media-create reliability — FIXED P0 + P1 + P2 (2026-06-24, NOT pushed).** Root causes (device-testing investigation, both confirmed in code): **(1, primary)** `POST /api/memories/create` **awaited the full AI cognition pipeline BEFORE the DB insert** (`generateMemoryInsights` 15s + `generateEmbedding` 15s, then `buildRelationships/clusters/people` via `Promise.allSettled`) with **no `export const maxDuration`** → the Vercel default function window killed the request → 504/"Something went wrong", and because the insert ran *after* the AI the **memory was lost entirely** (~95% device failure, size-independent). **(2, secondary)** media bytes are proxied through the function (server-side `supabase.storage.upload`), so the multipart body hits Vercel's **~4.5 MB** request limit → 413 for videos/large batches. **P0 FIX (insert-first + deferred enrichment):** create now **uploads → inserts with the user's title + neutral defaults → returns immediately**; the synchronous AI + cognition were **removed from the request**. New **`lib/memory-enrichment.ts`** (`enrichMemory` — idempotent, fault-isolated, UPDATE-based) runs insights→`ai_*`+title, embedding, and people/clusters/relationships; new **`POST /api/memories/[id]/enrich`** (`maxDuration=60`, owner-scoped) runs it in its **own** invocation budget; `CreateMemoryForm` fires it **fire-and-forget** after a successful save (`void fetch(...)`). Temp diagnostics: `CREATE_MEMORY_START`/`MEDIA_UPLOAD_START`/`MEDIA_UPLOAD_SUCCESS`/`MEMORY_INSERT_START`/`MEMORY_INSERT_SUCCESS` + `ENRICHMENT_START`/`ENRICHMENT_COMPLETE`/`ENRICHMENT_FAILURE` (userId/profileId/memoryId/attachment count+types). **⚠️ The ~4.5 MB body limit is NOT fixed by P0** — large videos/large multi-photo batches still 413; the real fix is **direct-to-storage upload (client→Supabase, API metadata-only)** — PLANNED, not implemented. **P1 (landscape):** mobile↔desktop nav swap moved `md` (768px) → **`lg` (1024px)** so landscape iPhones (≤~932px) keep the mobile nav — `AppNavbar` `hidden lg:flex`, `MobileTopBar`/`MobileBottomNav`/`MobileNavDrawer` `lg:hidden`, `WorkspaceSelector` drawer `max-lg:`/`lg:`, layout `<main>` `pb-24 lg:pb-6`. **P2:** `AttachmentManager` remove button `-right-1.5 -top-1.5` → `right-1 top-1` (was clipped by the tile's `overflow-hidden`). Frozen people/relationship/cluster/quota/reminder LOGIC unchanged — enrichment is the SAME code relocated off the request. tsc clean, lint **0 new** (2 pre-existing warnings), build **✓** (63/63; enrich route registered). **Device tests 1–8 are an operator step.**
- **RemyNest Branding System — FOUNDATION IMPLEMENTED (committed, NOT pushed).** Plan approved (two-tier identity / dark=tokens-only / staged a11y migration), foundation installment shipped. **Keystone: two-tier identity** — Product/Brand = sage/sand/gold (logo, app icon, OG, UI chrome); Companion/Remy = purple+gold pendant (in-app avatar ONLY); gold bridges. New: `lib/brand/tokens.ts` (source of truth); **SVG logos** `public/brand/{logo-mark,logo-mark-reversed,logo-horizontal,logo-stacked,remy-mark}.svg`; **generated assets** `app/icon.svg` + `app/apple-icon.tsx` (180 opaque) + `app/opengraph-image.tsx` (1200×630) via `next/og` inline SVG; `docs/brand/{brand-guidelines,typography}.md`. Edits: `tailwind.config.js` (`darkMode:'class'` + type scale + `gold-ink #7A5E22` — all additive), `app/globals.css` (**critical WCAG fixes: `p` color muted→text; focus ring gold→sage** + `.dark` token block), `app/manifest.ts` (theme_color `#000000`→`#4F6B5B`, bg `#f5f1e8`→`#F5F1EA`), `app/layout.tsx` (drop legacy `/favicon.png`+`/icon-192.png` icon refs → file-convention auto-wiring; `themeColor #4F6B5B`), CLAUDE.md authoritative brand note. **Dark = mechanism only** (not flipped on broadly). **Validated in-app Remy sprite + `remy-blueprint.png` untouched.** Lint **0 new** (67=baseline), build **✓**. ⚠️ **STAGED follow-ups (approved, not yet done):** raster exports (App Store 1024 / Play 512+adaptive / social / true-square PWA icons via a `sharp` script) — note `public/{favicon.png,logo-icon.png}` are 2.3 MB byte-identical dups to delete *after* a reference grep; the 17px/scale typography migration across ~544 `text-sm`/`text-xs` sites (staged + visually verified, not blind-swap); broad dark-UI component rollout + theme toggle + pre-hydration script; loading Fraunces into the OG route. See `docs/brand/brand-guidelines.md`.
- **Phase B — Media Expansion (video + mixed media) — IMPLEMENTED (committed, NOT pushed).** Evolves image-only memories into mixed-media (photo + video) by **unlocking video through the display surfaces** — the attachment architecture was already media-generic (`lib/memory-media.ts` already allowlists `video/*` + derives `type:"video"`; storage accounting is byte-based/type-agnostic; the upload pipeline is unchanged). Changes: **`AttachmentManager`** accepts `image/*` + `video/*` (type-aware previews — image thumbnail vs a play tile; no `<video>` mounted in the picker), **`MediaThumb`** renders video as a **play-icon tile** (so the feed `MemoryGalleryPreview` shows mixed media for free), **`PhotoViewer`** (`ViewerImage` gains `type`) renders **`<video controls playsInline preload="none">`** for video slides + a play tile in the strip — **±1 decode windowing preserved, no off-screen video decode**, prev/next + keyboard + swipe intact, **`MemoryGallery`** passes `type` through, and the **detail page** routes **image + video → the gallery/viewer** (audio/file stay inline). **Decision (per the "explain before changing structure" clause): evolved components IN PLACE, NOT renamed `PhotoViewer`→`MediaViewer` / `MemoryGalleryPreview`→`MediaGalleryPreview`** — `MediaThumb` is the real media seam; renaming frozen verified image components is cosmetic churn with no functional gain. **Limitation:** the **25 MB per-file cap applies to video** (kept — raising it touches the quota guardrail + Supabase limits; a video-specific cap is a separate decision). **Future audio/docs:** the pipeline allowlist + `MediaThumb` dispatch + byte accounting already accommodate them; only the picker accept-list + nicer players remain. Storage accounting/enforcement/usage-card/storage-full-modal all work for video unchanged (byte-based). Adversarially verified. Lint **0 new** (67=baseline), build **✓**. FROZEN systems (Stripe/billing/reminders/notifications/ledger schema/quota logic/existing image functionality) untouched. See `docs/features/media-system.md`.
- **Storage Usage UI + Upgrade Experience — IMPLEMENTED (committed, NOT pushed).** Surfaces the existing storage ledger/quota to users (reuse only — no accounting/quota/Stripe-backend change). New `components/storage/`: **`StorageUsageCard`** (full → **Settings** `/settings` Storage section; compact → **Dashboard** widget — plan/used/limit/remaining/percent/bar via react-query `GET /api/storage/usage`), **`StorageFullModal`** (surfaces the upload **HTTP 413 `{error, quota}`** — current/limit/remaining/projected — wired into the memories page **create + edit** mutations + dashboard **`CreateMemoryForm`**; optimistic rollback preserved; `memories/new` sends JSON-metadata only so it never 413s), **`StorageUpgradeModal`** (reusable; **web** shows the storage tier ladder + reuses **`POST /api/stripe/checkout`** — PREMIUM/FAMILY map to a Stripe price, STARTER "Coming soon"), **`ModalShell`** (portaled-to-body per the WebKit rule), `useStorageUsage`, and pure client-safe `lib/storage/format.ts`. **Apple 3.1.1/3.1.3 (authoritative):** every purchase entry point gated by `useIsNativePlatform()` + `isNativePlatform()`; on native iOS **no plans/prices/checkout/links/"manage on the web" steering** — only a neutral free-up-space message. ⚠️ **The task's native example "Manage your subscription on the web" was intentionally NOT used** — it violates the 3.1.3 anti-steering rule. Adversarially verified. Lint **0 new** (67=baseline), build **✓**. FROZEN systems (ledger schema/quota logic/Stripe backend/thumbnail/upload pipeline/reminders) untouched. ⚠️ Live numbers still need the ledger migration applied (operator); FREE 1GB default until billing→tier mapping. See `docs/features/storage-ledger.md`.
- **Multi-Image Detail Viewer (Phase 1 completion) — IMPLEMENTED (committed, NOT pushed).** Phase 1 (multiple images per memory) was largely already shipped (create/edit `AttachmentManager`; feed card `MemoryGalleryPreview` 1/2/3/4+ with +N; inline `MemoryGallery` swipe strip; `MediaThumb` future-media seam; thumb/medium signed-URL variants). This task closes the remaining **detail-viewer** requirements: **`PhotoViewer`** rewritten to add **prev/next buttons** (disabled at ends), **← / → keyboard nav** + Escape, and a **tappable thumbnail strip** (flex-col layout: track `flex-1` + strip `shrink-0`; active highlighted + auto-scrolled into view) — covering **mobile touch + desktop mouse/keyboard** (iOS-Photos/Google-Photos/FB-album behavior). The **±1 IntersectionObserver decode windowing**, `fallbackUrl` onError, portal-to-`document.body`, body-scroll-lock are **preserved**. The strip uses the small **THUMB variant** (`thumbnails` prop) + `loading="lazy"` so it never decodes 1080 medium images: the detail page signs a **second `signMemory(rawMemory,{variant:"thumb"})`** (bounded, one memory) and passes order-aligned `imageThumbnails` through `MemoryGallery`. Single-image memories unchanged (no strip/buttons). **No change to create/edit/upload/quota/ledger** (detail READ path + viewer only); **thumbnail signing layer untouched** (only *called* with the existing `variant` param). Adversarially verified. Lint **0 new** (67=baseline), build **✓**.
- **Upload Quota Enforcement — WIRED (committed, NOT pushed).** Connects the existing storage ledger + `checkStorageQuota` to the upload flow (this task **unfroze the upload pipeline**; ledger schema stays frozen). New `lib/storage/upload-guard.ts` `enforceUploadQuota(userId, files)`: sums the **total batch** bytes (`totalUploadBytes`), resolves pool members (`lib/storage/pool.ts` `resolveStoragePoolMembers` → `[userId]` today, **family seam**), calls **`checkStorageQuota`** (no duplicated accounting), returns the full UI payload (currentUsage/limit/remaining/projectedUsage/percentUsed, formatted + raw). Injected **before `buildMemoryMediaPayload`** (the storage-write choke point) in **`POST /api/memories/create`** + **`PUT /api/memories/[id]`** — covers every client upload path. Over-quota → **HTTP 413 `{ error, quota }`** (structured, never throws). **0-byte batch always passes** (text-only memory / remove-only edit never blocked); **fails closed** on a degraded read; edit path counts only **new** files (kept already in ledger); swap-in-one-edit is conservatively over-counted (safe). Adversarially verified. Lint **0 new** (67=baseline), build **✓**. Frozen systems untouched; no schema change. ⚠️ Still requires the ledger migration applied (operator) for live numbers; FREE 1GB default until billing maps tiers. See `docs/features/storage-ledger.md`.
- **Storage Ledger Foundation — IMPLEMENTED (committed, NOT pushed; migration operator-apply pending).** Accounting layer only — **NO billing/pricing/checkout** (those + the upload pipeline + thumbnail architecture are FROZEN this phase). New migration `supabase/migrations/20260623120000_storage_ledger_foundation.sql`: **`storage_ledger`** (one row/attachment: `user_id, workspace_id (=memory_profile_id, NULL=My Nest), memory_id FK CASCADE, attachment_id, file_size_bytes, media_type, created_at`, unique `(memory_id, attachment_id)`, RLS select-own) kept in sync by a **trigger on `memories`** (`AFTER INSERT OR UPDATE OF attachments, memory_profile_id, user_id`) projecting the `attachments` jsonb — **null/non-array/malformed-safe so it can never abort a memory write**; deletes via FK cascade. **Decision: maintained incrementally (trigger) over dynamic compute** — O(1) reads, and a trigger (not app dual-write) is required because the upload pipeline is frozen; `reconcile_storage_ledger()` does idempotent backfill + drift repair (run once after apply; the migration calls it). Fast read path: `storage_account_usage` (`security_invoker`) view. **`lib/storage/`**: `plans.ts` (FREE 1GB/STARTER 20GB/PREMIUM 100GB/FAMILY 100GB pooled — config, no pricing; decoupled from frozen `lib/billing`), `usage.ts` (`getStorageUsage` sums the view across a **member set**, service-role **explicitly scoped** by id — single-user today, family-pool-ready with no redesign), `enforcement.ts` (`checkStorageQuota` → structured `{allowed, projectedBytes, overageBytes, usage}`, never throws, byte-based → future-media-proof). APIs: **GET `/api/storage/usage`**, **POST `/api/storage/check`** (auth-gated; the check is a primitive, **not** wired into the frozen upload pipeline). Lint **0 new** (67=baseline), build **✓**. ⚠️ **OPERATOR STEP:** apply the migration in the Supabase SQL editor (dashboard-managed); dormant until then. See `docs/features/storage-ledger.md`.
- **Thumbnail Architecture (Phase 0) — IMPLEMENTED (committed, NOT pushed).** The durable image-decode OOM fix. `lib/memory-media-signing.ts` now mints a **per-context size ladder** from the single stored original: **THUMB** `400×400 cover q70` (feed: `MemoryGalleryPreview`, `CompactMemoryRow`, `TimelineRow`) and **MEDIUM** `1080 contain q75` (detail `MemoryGallery` + `PhotoViewer`). Images use **Supabase on-the-fly transforms** via the SINGULAR `createSignedUrl(path, ttl, {transform})`; the BATCH `createSignedUrls` (untransformed) is always run first as the **hard fallback**. `signMemories`/`signMemory` gained `{ variant, maxImagesPerMemory }`; feed routes + timeline page pass `thumb` (cap 4), the detail page passes `medium`; **default callers (search/create/reminiscence) unchanged** (variant optional). Each image carries `fallbackUrl` (untransformed) → `MediaThumb` (2-stage), `PhotoViewer` (per-slide failed-set), `CompactMemoryRow` recover from a transform render failure (>25 MB/50 MP). `toStoragePath` strips `/render/image/sign/` URLs so edits never persist a render URL. **Pagination:** `/api/memories` + `/api/timeline` take `limit`/`offset` (`.range`, default 50) to bound the singular-signing fan-out; the memories feed client **aggregates pages into its flat array** (optimistic create/edit/delete mutations untouched). ⚠️ **OPERATOR GATE (required to activate):** Supabase Image Transformations (**Pro plan**) **and** env **`MEMORY_IMAGE_TRANSFORMS_ENABLED=true`** (Vercel). **Default OFF → deploy is byte-identical** (no broken images, no regression) until enabled. Adversarially verified (4-reviewer workflow, verdict **fix-then-ship**): fixed the one flag-independent must-fix — added an **`id` ORDER BY tiebreaker** to both paginated routes + the timeline page so offset paging is **stable** across the client's separate page requests (`created_at` is not unique → could otherwise drop/duplicate a memory past 50 rows). Lint **0 new** (67=baseline), build **✓**. FROZEN systems untouched. **⚠️ Pre-activation checklist (do BEFORE flipping `MEMORY_IMAGE_TRANSFORMS_ENABLED=true` — all inert while OFF):** `TimelineRow` (raw `next/image`, no `onError`/`fallbackUrl`) → give it the `CompactMemoryRow` fallback pattern; `TimelineCard`/`TimelineAttachmentImage` + `MemoryCoverImage` → try `fallbackUrl` before the placeholder; cover (`cover_image_url`, scalar column) has no untransformed recovery under transforms-on (acceptable: labeled placeholder). **Follow-ups:** video poster / PDF first-page stored derivatives (reuse `attachment.thumbnailUrl`); keyset/cursor pagination + feed virtualization at very high counts.
- **Launch Fast Slice (2026-06-23, NOT pushed).** REPO-only launch-blocker pass (no branding/Android/legal/store work — separate tracks; no FROZEN-system changes). **(1) Landing CTA wiring** (`components/marketing/LandingClient.tsx`): the 4 dead store/download `<button>`s (App Store, Google Play, Download iPhone/Android) now wrap `<a href="/download">`; footer Privacy → `/privacy`, Security → `/support` (no `/security` route; repointed to the support page's security section); navbar Features/Security/Caregivers → in-page anchors `#how-it-works`/`#private-secure`/`#caregivers` (matching `id`s added). **(2) Compliance copy** (same file): removed the fabricated **98% Memory Confidence** card + all medical/clinical framing ("Cognitive/Healthcare/Human Continuity"); deferred features converted to **roadmap language** — Voice Memories + AI Insights tiles now show a **Coming Soon** badge; "Semantic" relabelled to the shipped **Smart Search / AI Summaries**; "Second Brain" → "Preserve What Matters Most"; copy aligned to "A safe place for your memories". Colors untouched (branding track). **(3) Subscription nav** (`components/profile/config/profile-menu.config.ts`): added a **Subscription** menu item → `/account/subscription` (the page was orphaned/unreachable). **(4) Billing consolidation:** `/account/subscription` is now canonical — it renders the full **`BillingSection`** (status/renewal/trial/portal/upgrade/cancel, all 3.1.1-gated) alongside `StorageUsageCard`; the duplicate inline ProfileHub **"Billing" section was retired** (removed from `profile-sections.config.ts`) and the thinner `SubscriptionBilling.tsx` was **deleted**. All Stripe wiring + native gating reused unchanged — no billing-logic/backend/plans/quota edits. Lint **0 new** (67=baseline), build **✓** (`/account/subscription` compiled). **Note:** the landing page is still off-palette cyan/blue (branding track) and the page now shows two upgrade affordances (StorageUsageCard's modal + BillingSection's) — acceptable, both gated.
- **Public launch funnel (2026-06-23, NOT pushed).** 4 new routes, reusing existing infra — no changes to reminders/push/storage/billing-backend/gallery/uploads. **`/pricing`** (public) renders FREE/PREMIUM/FAMILY straight from `BILLING_PLANS` (storage/care-profiles/capabilities derived, **no duplicated values**); CTA `components/marketing/PricingActions.tsx` reuses `POST /api/stripe/checkout` and is **Apple 3.1.1/3.1.3 gated** (`useIsNativePlatform`/`isNativePlatform` → on native: neutral in-app link only, no checkout/external link/steering; logged-out web → `/signup`). **`/download`** (public) iOS/Android/Web sections; store URLs from `NEXT_PUBLIC_APP_STORE_URL`/`NEXT_PUBLIC_PLAY_STORE_URL` with a neutral **Coming soon** fallback (no dead links); web → `/login`. **`/support`** (public) uses `docs/compliance/04` copy — support/privacy/DPO/security contacts (added to `lib/contact.ts`) + the data-deletion path (`/account-deletion`). **`/account/subscription`** (authenticated, under the `(app)` group) surfaces current plan + storage used/limit via the existing `StorageUsageCard`, plus billing-portal access via `components/account/SubscriptionBilling.tsx` (reuses `POST /api/stripe/portal`, **web-only** per 3.1.1 — null on native; FREE/no-customer → "See plans"). Added `/pricing`,`/download`,`/support` to `middleware.ts` `PUBLIC_ROUTES`. **No new billing logic.** Lint **0 new** (67=baseline), build **✓** (all 4 routes compiled). **Operator follow-up:** stand up the `support@/privacy@/dpo@/security@` mailboxes, set the store-URL env vars at submission, wire the landing store buttons to `/download`.
- **Branding Asset System (2026-06-23, NOT pushed).** Branding/launch assets only — no app/feature/billing/storage/reminders changes. **Created:** dark logo package `public/brand/logo-{mark,horizontal,stacked}-dark.svg` (sand arcs + moss/sand wordmark for dark surfaces); **6 static Remy state SVGs** `public/brand/remy-{happy,thinking,listening,celebrating,concerned,sleeping}.svg` (geometric, no animation — animation is post-launch); **`scripts/generate-brand-assets.mjs`** (sharp raster generator from the SVG masters → true-square PWA icons, App Store 1024 opaque, Play 512 + adaptive, maskable, favicon PNGs); **`docs/brand/asset-production.md`** (Phase 2-4 icon/store/marketing matrix + audit + missing list). **Updated** `brand-guidelines.md` (dark variants, Remy avatar states, spacing). **Audit:** the existing `public/{icon-192,icon-512,favicon,logo-icon}.png` are **broken 1536×1024 non-square** dups; native iOS/Android icons + splash exist but predate the nest brand. **⚠️ Operator-pending (env has no `sharp`):** `npm i -D sharp && node scripts/generate-brand-assets.mjs` → wire native icons (`@capacitor/assets` + `cap sync`) → delete the non-square dups → capture store screenshots (📸) + illustrated marketing (🎨). Lint/build unaffected (assets + docs only). See `docs/brand/asset-production.md`.
- **Family / care-profile reviewer-experience fixes (2026-06-24, NOT pushed).** UI-only — no billing/subscription/reminder/push/ledger/memory-CRUD/family-permissions backend changes. **(T1+T2, CRITICAL)** `components/InviteCaregiverForm.tsx` now checks the **caregiverCollaboration entitlement BEFORE rendering the form** (via `useStorageUsage` tier + `BILLING_PLANS`): no access → a neutral "Caregiver Collaboration" info state (native: text only, **no purchase UI/link**; web: a "See plans" → `/account/subscription`). Removed the post-submit `UpgradeModal` from this path — so the native **"Family plan feature / Got it" dead-end is now unreachable** (verified: no `reason:"caregiver-collaboration"` caller remains). **(T3, HIGH)** new dismissible `components/profile/MyNestExplainer.tsx` (My Nest vs Care Profiles, plain language, `useSyncExternalStore` localStorage) shown on `/home` only when `getAccessibleProfiles().length===0`. **(T4, HIGH)** added **"People" → `/profiles`** to `nav-config.ts` (drawer + desktop; primary tabs unchanged) — the People directory is now discoverable. **(T5, HIGH)** new `components/profile/people/AddPersonButton.tsx` — inline "Add a Person" on the `/profiles` empty state + header opens `CreateProfileForm` in a modal (no hidden-menu redirect). **(T6, HIGH)** `CreateProfileForm` copy → **"Add a person" / "Their name" / "Add Person"** (UI only; `name="profile_name"` DB field unchanged). **(T7, MEDIUM)** `CreateProfileForm` success → **`router.refresh()` + `onSuccess()` close** (drawer/modal) instead of `window.location.reload()` — no white-flash. Lint **0 new**, build **✓**.
- **Reviewer-experience: HIGH/MEDIUM fixes — onboarding, success feedback, empty states (2026-06-24, NOT pushed).** **(T1 onboarding polish)** `app/(app)/onboarding/page.tsx` rebranded to the design system — `bg-black`→`bg-sage`, `focus:ring-black`→`focus:ring-sage`, raw `#f5f1ea`/`#2f3e34`→`bg-sand`/`text-charcoal`, added `<RemyNestLogo>`. No new steps; server-action logic untouched. **(T2 onboarding enforcement)** signup + reset-password redirects `/memories`→**`/home`** (which already enforces the onboarding gate); login already → /home. All 3 auth entry points now route new users through onboarding; loop-safe (no new gate; /home→/onboarding, /onboarding has no gate). **(T3 success feedback)** Mounted the unused **`ToastProvider`** in `app/layout.tsx` (inside QueryProvider). `showToast("Memory saved")` on memory create (`CreateMemoryForm` + the feed `createMutation`) and `showToast("Person added")` on `CreateProfileForm` (700ms before its reload). **Reminder toast deferred** — `createReminder` is a FROZEN `"use server"` action that already revalidates (the reminder appears in the list); adding an explicit toast would mean touching frozen reminder code. **(T4 empty states)** `/home` (welcome + "Add your first memory" when `coverage.total===0`), memories feed (styled card + "Add your first memory" → opens create modal via `setShowCreate`), `/timeline` (neutral copy — removed the confusing "AI timeline filters" wording — + CTA), `/profiles` (clearer "add from the workspace menu" guidance). No marketing/AI/future-feature copy. Lint **0 new**, build **✓**. No billing/subscription/reminder-logic/push/ledger/memory-CRUD-backend/family-permissions/AI changes.
- **Reviewer-experience: 3 critical fixes (2026-06-23, NOT pushed).** Closed the 3 Apple-rejection-class findings from the reviewer audit — UI logic only, no backend/frozen-system changes. **(C1, Apple 2.1)** Removed 4 placeholder profile-dropdown sections (Security/Notifications/Vault/Caregiving — zero working controls) from `profile-sections.config.ts` + deleted the files; the ProfileHub dropdown is now just **Account** (which links to the real `/settings`) + **Privacy & GDPR**. The functional settings live at `/settings` (AccountInfo/Storage/Export/PrivacyLinks/DeleteAccount) — untouched. **(C2, broken create FAB)** The mobile "+" FAB (`nav-config.ts` → `/memories/new`) hit an orphan form that POSTed file **metadata only** (photos silently dropped). `app/(app)/memories/new/page.tsx` now renders the **production `CreateMemoryForm`** (multipart `/api/memories/create`, real bytes, AttachmentManager, 413 handling) — text/single/multi-photo all work. Deleted the unrelated orphan `app/(app)/reminders/new` (broken stub that inserted a memory; no links). **(C3, Apple 2.1/3.1.1)** `BillingSection.tsx` gated Manage/Portal/Cancel behind `hasActiveSubscription` (`currentPlan !== "FREE"`): FREE now shows **"Free plan"** (was "FREE • inactive") + storage + (web) upgrade options only — **no Cancel/Portal**; native stays a neutral Premium state. No Stripe/billing-backend change. Lint **0 new**, build **✓**.
- **Branding asset audit + PWA maskable wiring (2026-06-23, NOT pushed).** Full asset audit (no new branding). Closed the one open REPO gap: `app/manifest.ts` now declares the square PWA icons `purpose:"any"` + the **maskable variants** (`/brand/icons/maskable-192/512.png`, `purpose:"maskable"`) — Android/PWA installs now get a safe-zone adaptive icon. Verified: favicon.ico/favicon.svg(`app/icon.svg`)/icon-192/512/maskable/apple-touch(`apple-icon.tsx`)/App-Store-1024/Android-adaptive-fg+bg/OG(`opengraph-image.tsx`, sage nest) all present + correct; no broken paths; deleted dups stay gone. **Flagged (not changed — pre-existing SEO infra, out of asset scope):** `lib/seo.ts` `SITE_OG_IMAGE="/logo.png"` is a stale 596×180 wordmark used as the og/twitter fallback while the proper dynamic OG card exists — recommend repointing/regenerating later (it's also dual-used by the `app/page.tsx` JSON-LD logo). Build **✓**, lint 0 new. Native iOS/Android icon+splash wiring (operator/`@capacitor/assets`) + designer screenshots/feature-graphic remain.
- **Branding raster pipeline COMPLETE + RUN (2026-06-23, NOT pushed).** Finished the sage-nest asset pipeline (no new branding; product brand unchanged). **Added the missing generation logic:** `scripts/generate-brand-assets.mjs` now assembles **`app/favicon.ico`** in-script (modern embedded-PNG ICO from the 16/32/48 buffers — no new dep; refactored `icon()` to share a `compose()` buffer helper). **Installed `sharp` (devDep) and RAN the generator** → produced from `logo-mark-dark.svg` (sage nest): true-square **`public/icon-192.png`/`icon-512.png`** (fixing the broken 1536×1024 dups), **maskable-192/512**, **app-store-icon-1024**, **play-icon-512**, **play-adaptive-foreground/background-432**, **favicon-16/32/48**, and the rebranded **`app/favicon.ico`** (was the Create-Next-App default). Deleted the stray non-square `public/{favicon,logo-icon}.png` dups. Build **✓**; lint 0 errors (the prior "67/2-errors" was a degraded result from an incomplete node_modules — the completed install resolves the eslint config to 2 pre-existing warnings in a validation script). **Remaining (not generation, flagged):** wire native iOS/Android icons + splash from these masters via `@capacitor/assets` + `cap sync` (operator/native), and add `purpose:'maskable'` to `app/manifest.ts`. Designer-only: store screenshots + Play 1024×500 feature graphic. See `docs/brand/asset-production.md`.
- **REPO launch-readiness pass (2026-06-23, NOT pushed).** Five REPO-owned items from the Launch Readiness Audit — no frozen-system changes. **(1) Doc truth pass:** `docs/compliance/{01,08,09,12}` rewritten to match reality — **email/password auth only** (Sign in with Apple **N/A**, no Google login), **web-only Stripe** subscriptions (no Apple IAP, **no Restore Purchases**), native 3.1.1 purchase gating, no social login. **(2) Auth polish:** new `components/brand/RemyNestLogo.tsx` (product **nest** lockup) added to `app/(auth)/layout.tsx` (shows on all auth screens); login→signup + signup→login links; **deleted orphan `app/(auth)/register/page.tsx`**; signup upgraded (real `<form>`, `type=email`+autocomplete, loading spinner, **email-confirmation** state). **(3) Storage freshness:** delete (`memories/page.tsx` deleteMutation `onSettled`) + standalone create (`CreateMemoryForm`) now invalidate `["storage-usage"]` (create/edit feed paths already did) → usage card/banner refresh immediately after create/edit/delete. **No** storage-accounting/ledger/quota change — client cache invalidation only. **(4) Android source:** `POST_NOTIFICATIONS` added to `AndroidManifest.xml`; `ic_launcher_background.xml` `#FFFFFF→#4F6B5B` (sage). **(5) `npx cap sync android`:** regenerated `capacitor.{build.gradle,settings.gradle}` — removed the **stale broken `onesignal-cordova-plugin` gradle ref** (not a dep; was failing Gradle sync) and **linked `@capacitor/local-notifications`**. iOS native/Podfile/AppDelegate UNTOUCHED (cap sync android only). ⚠️ Note: cap sync also dropped the orphaned Android OneSignal SDK line (already non-functional — no FCM); Android push is re-established by the operator's FCM/keystore fast-follow. Lint **0 new** (67=baseline), build **✓**.
- **Multi-image: image REORDER added (2026-06-23, NOT pushed).** Closed the one remaining gap in the multi-image feature: `components/memories/AttachmentManager.tsx` now has per-tile **move-earlier/move-later (‹ ›)** controls (shown when a group has >1 item), reordering both **existing** saved attachments (`onExistingChange`) and **new** files (`onFilesChange`). Order persists through the **unchanged** save flow — PUT `/api/memories/[id]` composes `attachments` from `kept` (array order) + new files via `buildMemoryMediaPayload`; **byte-neutral** (the ledger trigger projects bytes per `attachment_id`, order-agnostic — no schema/quota/ledger change, no migration). Everything else in multi-image was already shipped + verified: multi-select create, detail swipeable `PhotoViewer` (swipe + prev/next + ←/→ + **`1/N` counter**), card grids (2/3/4+ with `+N`), mobile-feed grid, byte-based storage, cross-workspace. Dependency-free (no drag-drop lib). Lint **0 new** (67=baseline), build **✓**. No reminders/storage-ledger/billing/upload-pipeline changes.
- **Media Expansion — mobile-feed grid completion (2026-06-23, NOT pushed).** The condensed photo-grid preview (2 side-by-side · 3 large+two-stacked · 4+ grid with +N) was desktop-only (`MemoryCard` → `MemoryGalleryPreview`); the mobile feed (`CompactMemoryRow`) showed only a single thumb + count. Now `CompactMemoryRow` renders **`MemoryGalleryPreview`** below the row for memories with **≥2 image/video attachments** (the leading 48px thumb is hidden when the grid shows) — so the grids are visible **mobile-first**, tapping → the detail's full swipeable `PhotoViewer`. **Single-image + text-only rows unchanged (backward compatible).** The rest of media expansion (multi-photo create/edit; the 1/2/3/4+ grids; detail swipe gallery + tap-to-expand; mixed image+video via `MediaThumb`; byte-based storage accounting) was already shipped + adversarially verified. No change to reminders/notifications/Stripe/quota/billing. Lint **0 new** (67=baseline), build **✓**. **Remaining media: audio/document/PDF uploads = POST-LAUNCH.**
- **Memory Gallery Experience (Phase 3) — IMPLEMENTED (committed, NOT pushed).** Four new display components: **`MediaThumb`** (type-dispatch seam: image→`next/image` lazy/`unoptimized`+onError; video/audio/doc→typed placeholder — **future-media-ready**, no redesign), **`MemoryGalleryPreview`** (Facebook-style card layouts 1/2/3/4+ with a `+N` tile, ≤4 lazy tiles), **`MemoryGallery`** (detail-page horizontal **CSS scroll-snap** swipe strip → opens the viewer on tap), **`PhotoViewer`** (full-screen, **portaled to `document.body`** per the CLAUDE.md WebKit rule, scroll-snap swipe, close+position, **windowed decode — only current ±1 slide mounts an `<Image>` via IntersectionObserver**, so ≤3 full-res bitmaps resident → the WKWebView OOM mitigation). Wired: `MemoryCard` (flat 3-grid → `MemoryGalleryPreview`; dropped unused `Image`/`useState`/fallback), `app/(app)/memories/[id]/page.tsx` (images→`MemoryGallery`, video/audio/file inline branches unchanged), `CompactMemoryRow` (+N thumbnail cue + `loading="lazy"`). **Display-only — no DB/schema/pipeline/`cover_image_url` change; reminders/billing/storage untouched.** Lint **0 new** (67=baseline), build **✓**. ⚠️ The gallery decodes more images → **amplifies the image-decode OOM**; windowed-decode + lazy + 4-tile cap mitigate it, but the **server-side thumbnail transform (Phase 0, operator-gated) is the durable fix — recommended next**. **Out of scope (still future):** pinch-zoom, real video/audio/PDF rendering, pagination.
- **Memory multi-photo create + edit (Phase 1 + 2) — IMPLEMENTED (committed, NOT pushed).** New shared **`components/memories/AttachmentManager.tsx`** (thumbnail preview + per-item remove + "Add photos", image-only, mobile-first, `next/image` `unoptimized`). Wired into **create** — memories-page `CreateMemoryModal` (was a bare multi-input → now preview/remove) and dashboard `CreateMemoryForm` (had **no** photo support → now multi-photo via a FormData branch to `/api/memories/create`) — and **edit** — `EditMemoryModal` now shows existing photos with remove + add new. **`PUT /api/memories/[id]`** gained a FormData branch reusing **`buildMemoryMediaPayload`** (uploads new files + merges with kept attachments via the existing pipeline; recomputes `cover_image_url` = first image); the JSON path is unchanged for backward compat; `updateMutation` switched to FormData. **Fixed a pre-existing data-loss bug:** the old JSON edit sent no `attachments` → `normalizeAttachments(undefined) = []` → **every edit WIPED a memory's photos + cover**; the new edit preserves/manages them. Order preserved (kept first, new appended). **No schema/migration/AI/search/reminder changes** (AI + search are text-only; attachments never reach them). Lint **0 new** (67=baseline), build **✓**. **Out of scope (future phases):** gallery grids, Facebook cards, carousel, full-screen viewer, thumbnails, pagination, video/audio/PDF. **Post-audit hardening (`ce39a8c` follow-up):** `EditMemoryModal` now has try/catch/finally + a visible error (loading always resets); both create/edit modals get `max-h-[90vh] overflow-y-auto` so Save/Cancel stay reachable on mobile with large photo grids. **Verified clean:** M1 — the upload pipeline always sets `id`/`type`/`filename`, so no normal attachment is dropped by `normalizeAttachments` (only hypothetical pre-pipeline legacy rows, operator-verifiable); M2 — `next/image` (14.2.5, `get-img-props.js:226`) auto-handles `blob:`/`data:`, so previews render.

...
- **My Nest (PERSONAL) reminders — ENABLED in code; gated on operator DB migration (committed, NOT pushed).** Reminder Center was care-profile-only; it now honors the app-wide `memory_profile_id IS NULL + user_id` convention (same as memories/timeline/search). **`app/(app)/reminders/page.tsx`:** removed the My-Nest early-return + the create throw; PERSONAL reads/writes scoped `.is("memory_profile_id", null).eq("user_id", uid)`, CARE unchanged (`.eq("memory_profile_id", activeProfileId)` + `userCanAccessProfile`). **Hardened `app/(app)/reminders/[id]/page.tsx`** (was an unscoped `select * eq(id)` → a cross-user read once personal rows exist): added auth + app-level authz (personal→`user_id` owner; care→`userCanAccessProfile`), independent of RLS. **Migration `supabase/migrations/20260621120000_reminders_personal_mynest.sql`** (operator-apply): `DROP NOT NULL` on `memory_profile_id` + 4 additive personal-owner RLS policies; idempotent; does **not** toggle RLS enablement. Validated: lint 0-new, build ✓; adversarially reviewed (no cross-user/cross-context leak; CARE byte-identical). **DEPLOY ORDER (hard):** operator verifies `reminders.memory_profile_id` nullability + RLS state in the Supabase SQL editor → applies the migration → **THEN** push the app code (`main` auto-deploys). On-device: My Nest reminders schedule + fire via the existing `NativeReminderSync` (no profile dependency).
